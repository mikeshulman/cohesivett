...
a logic paramterized by a 2-category of modes, where each mode
...
of the rule.  
%% The direction of the 2-cell, and the fact that it may not
%% be invertible, are chosen to match the intended semantics and permit the
%% proofs of the desired admissible rules.  
...
Next, we consider a further refinement on the walking reflection mode
...
walking idempotent mode theory, then we would like them to be sound (can
...
specialized calculus, and an equivalent natural deduction system, treats
positive types more similarly to how they are traditionally handled in
intension type theory, and corresponds closely to the rules for spatial
...
map into a discrete type).  The restriction from the general adjoint
logic rules is that we can, without loss of generality, keep the
crispness of $A$ in the premise---there is no reason to demote it to a
...
either case we know $A$ under the discrete retopologization.  The
restriction from the general adjoing logic rules is that we always
...
\subsection{Avoiding left-commutative cut reductions}

FIXME: wrong story here; talk about crisp-only rules up front.

All of the sequent calculi that we have defined so far, including the
above restricted sequent calculus for this mode theory, are very nice
...
goal $D$, so we do not need to reduce a left-commutative cut.  For
infinite types such as the natural numbers, it seems necessary to build
in these cuts, to allow strengthening the induction formula (unless one
uses an infinitely wide proof tree with an $\omega$-rule).

There are good arguments for both including and not including the left
commutative cut reductions as definitional equalities (they will be true
for the identity type in any case): If we include them, then at least
for fragments consisting only of finitary types, the calculus has the
subformula property, identity and cut can be shown admissible by
structural induction on types, and more terms are equal so more terms
type check.  On the other hand, if we do not include them, then the
syntax can be interpreted in models where they do not hold strictly, the
rules for finite and infinite positives are more uniform with each
other, and with the rules for higher inductive types.  Since it seems
likely that the left-commutative cut reductions will not be strict
equalities in some of the desired models of cohesive homotopy type
theory (e.g. where positives involve a fibrant replacement), it is worth
investigating an alternative sequent calculus in which the
left-commutative cuts are not reduced.

One way to accomplish this would be to take the sequent calculus from
Section~\ref{sec:idempotent-calc1}, add cut as a derivable rule, and
then define a normalization algorithm that reduces right-commutative and
principal but not left-commutative cuts.  However, when the
left-commutative cuts are internalized, it turns out that certain
instances of the left rules for positives become provable from more
restricted rules.  We use the following sequent calculus
\rseq{A}{\alpha}{B} to explain this idea.  For simplicitly, we make all
cuts derivable, and defer to future work an investigation of principal
and right-commutative cut reduction, because the stuck left-commutative
cuts prevent using the usual structural argument~\citep{pfenning94cut}.
...
the branches.  In the presence of cut and identity rules, we can derive
the more general left rules for crisp positive assumptions
...
from \Flat{}-induction, even though it was not necessary to get crisp
...
The advantage of these restricted elimination constants is that
analogous rules can be used for all colimits, inductive types, and
higher inductive types, and the  
...
for positives with only $\beta$-rules was considered in
~\citet{lh05dmlproofs}).
