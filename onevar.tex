...
\paragraph{$F$ and $U$ are functorial on maps.} If $D : \seq{A}{1}{B}$ then
...
\seq{\U \alpha A}{1}{\U \alpha B}$.  Moreover, $\ap{\F \alpha
  ...
  {({D}\cuti{E})}}{{\F \alpha {D}} \cuti {\F \alpha {E}}}$, and
analogously for $U$.  It follows that $F$ and $U$ preserve isomorphisms:
If $(D,E) : A \cong B$ then $(\F \alpha D,\F \alpha E) : \F \alpha A \cong \F \alpha B$
and $(\U \alpha D,\U \alpha E) : \U \alpha A \cong \U \alpha B$.  

\paragraph{$F$ and $U$ are adjoint.}: If $D : \seq{\F \alpha A}{1}{B}$ then there is
a $D' : \seq{A}{1}{\U \alpha B}$, and vice versa, and round-triping the
two is the identity on $D$ up to $\ap{}{}$.
FIXME: naturality

\paragraph{The functors $F_\alpha$ and $U_\alpha$ are determined
  functorially in $\alpha$.} First, on objects we have $F^1 : \F {1}{A}
\iso A$ and $U^1 : \U {1} A \iso A$ and $F^\circ :
\F{\alpha}{\F{\beta}{A}} \iso \F {\compo{\beta}{\alpha}} A$ and $U^\circ
: \U \beta {\U \alpha A} \iso \U {\compo{\beta}{\alpha}} A$.  Next, on
morphisms, we have $\F{\alpha}{D}$ and $\U{\alpha}{D}$ are functorial in
$\alpha$, up to the isomorphisms above: \ap{\F {1} D} {{F^1} \cuti {D}
  \cuti {\inv{F^1}}} and \ap{\F {\beta} (\F \alpha D)} {F^\circ \cuti \F
  {\compo{\alpha}{\beta}} D \cuti \inv{F^\circ}}, and similarly for $U$.
