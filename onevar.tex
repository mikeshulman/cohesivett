...
focusing~\cite{andreoli92focus}, we conjecture that the connectives can
be given the same focusing behavior as in \cite{reed09adjoint}: $F$ is
...
limits, because can be written either as $U_{\dsd d}$ and $F_{\dsd n}$;
...
and details througout, a full proof of soundness
(Section~\ref{sec:semantics}), and specialized sequent calculi for a
specific mode theory (\ref{sec:idempotent}), which clarify the
...
\seq{A}{\alpha}{C} is intended to mean either/both of these.  Wehn
