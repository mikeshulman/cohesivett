...
\compo{\dsd{d}}{\dsd{n}}$, so we could equivalently work in the
\dsd{c}-types above.

Now we define $\Flat{} := \F{\dsd{r}}{}$ and $\Sharp{} := \U{\dsd{r}}{}$
and immediately have $\Flat{} \la \Sharp{}$, which required proof above
(but on the other hand, it now requires proof that they are
monads/comonads).  For this mode theory, we can without loss of
generality use the following instances of the sequent calculus rules:
\[
\begin{array}{c}
\infer[\ident{A}]
      {\seq{P}{\dsd{1}}{P}}
...
\end{array}
\]
For \irl{FL} and \irl{UR}, whether we have a 1 or an \dsd{r} in the
conclusion, we will compose with \dsd{r} in the premise, giving an
\dsd{r} premise in both cases.  For \irl{FR}, a general instance would
have the form
\[
\infer[\irl{FR}]
      {\seq {C} {\beta} {\F {\dsd{r}} A}}
      { \tc{\compo{\gamma}{\dsd{r}}}{\beta} &
        \seq {C} {\gamma} {A}}
\]
However, because $\compo{\gamma}{\dsd{r}} = \dsd r$ for both $1$ and
\dsd{r}, $\beta$ must be \dsd{r}, and $\gamma$ can be $1$ or \dsd{r}.
But since an \dsd{r}-assumption is stronger than a 1-assumption, because
\seq{C}{\dsd{r}}{C}, there is no reason to ``demote'' the assumption at
this point, and we can without loss of generality choose $\gamma =
\dsd{r}$.  (FIXME: explain that this works out equationally, too).  The
same reasoning applies to \dsd{UR}.

... get to natural deduction rules eventually ... 
