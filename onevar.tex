...
\newcommand\semltor[1]{\ensuremath{#1^{\vartriangleright}}}
\newcommand\semrtol[1]{\ensuremath{#1^{\vartriangleleft}}}
...
pseudofunctor as follows, writing
\begin{itemize}
\item $\C_p$ for $S(p)$

\item $\semFo{\alpha}{} \la \semUo{\alpha}$ for $S(\alpha)$, and
  \semltor{-} and \semrtol{-} for two maps of hom-sets of the adjunction

\item $\semFo{e} : \arrow{}{\F \beta A}{\F \alpha A}$
...
$\semltor{m};\semUo{e} =_{\arrow{}{A}{\semU{\beta}{B}}} \semltor{(\semFo{e};m)}$,
or equivalently that 
for any $m : \arrow{}{{A}}{\semU{\alpha} B}$, we have
$\semFo{e};\semrtol{m} =_{\arrow{}{\semF \beta A}{B}} \semrtol{(m;\semUo{e})}$.

\item $\semFone : \semF{1}{A} \iso A$ and $\semUone : \semU{1}{A} \iso
  A$ for the components of the two natural isomorphisms in the
  adjunction isomorphism $S^1$ between $\semFo{1}{} \la \semUo{1}$ and
  the identity adjunction.

\item $\semFcomp(\beta,\alpha) : \semF{\compo{\beta}{\alpha}}{A} \iso
...
\[
\infer[\irl{UL}]
      {\seq {\U {\alpha : r \ge q} A} {\beta : r \ge p} {C}}
      { \gamma : q \ge p &
...
\semltor{\sem{D}} : \arrow{}{A}{\semU \gamma C} and it suffices to give
\arrow{}{{\semU {\alpha} {\sem A}}}{\semU {\beta} \sem C}.  So we form
the composite
\begin{diagram}
{{\semU {\alpha} {\sem A}}} & \rTo^{\semU{\alpha}{(\semltor{\sem{D}})}} &
...
\sem{\UR {D}} & := & \semltor{(\inv{\semFcomp(\beta,\alpha)};{\sem{D}})} \\
\sem{\UL {\gamma}{e}{D}} & := & \semrtol{({\semU{\alpha}{(\semltor{\sem{D}})}} ; {\inv{\semUcomp(\alpha,\gamma)}} ; {\semUo{e}})}
\end{array}
\]
\end{proof}
...
\ltor{(\inv{\semFcomp(\beta',\alpha)};\semFo{\compv{e}{1_\alpha}};\sem{D})}{}$
...
\item Case for $P$: $\semFone_P = \sem{\hyp 1}$
...
\begin{lemma}
For all derivations $D$ and $E$, $\sem{\cut D E} = {\semFcomp(\beta,\alpha)};{\semF{\alpha}{\sem{D}}};{\sem{E}}$
\end{lemma}

\newcommand{\semofcut}[2]{{\semFcomp(\beta,\alpha)};{\semF{\alpha}{\sem{#1}}};{\sem{#2}}}
...
\item $\semofcut {(\UL \gamma e D)} E = \sem{\UL {\compo{\gamma}{\alpha}}
...
\begin{theorem}[Soundess of the equational theory.] If \ap{D}{D'} then $\sem{D} = \sem{D'}$.  
\end{theorem}
