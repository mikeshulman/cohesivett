...
\begin{lemma}
For all derivations $D : \seq{A}{\beta}{B}$ and $E : \seq{B}{\alpha}{C}$, $\sem{\cut D E} = {\semFcomp(\beta,\alpha)};{\semF{\alpha}{\sem{D}}};{\sem{E}}$
\end{lemma}
...
\[
\semFcomp(\beta,\alpha);\semF{\alpha}{(\semF{e};\semFone)};\semF{e'};\semFone = \semFo{\comph{e}{e'}};\semFone
\]
By functoriality, $\semF{\alpha}{(\semF{e};\semFone)} =
\semF{\alpha}{\semF{e}};\semF{\alpha}{\semFone}$, and by naturality of
\semFo{e'}, $\semF{e'};{\semF{1}{\semFone}} =
\semF{\alpha}{\semFone};\semFo{e'}$,
so the LHS equals
\[
(\semFcomp(\beta,\alpha);\semF{\alpha}{\semFo{e}};\semF{e'});{\semF{1}{\semFone}};\semFone
\]
