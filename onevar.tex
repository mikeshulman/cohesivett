...
%% FIXME: don't overload Delta and Nabla
...
%% FIXME:
%% Maybe a better way to put it is that the thing that's a bit unusual here
%% is to index a syntactic judgement by a semantic structure---we wouldn't
%% be able to encode this directly in LF, e.g., and it's possible for the
%% "definitional" equality of the syntax to be undecidable if we choose a
%% mode category that includes undecidable word problems on groups.  A more
%% syntactic way to do it would be to define a syntax for 2-categories, so
%% that p and alpha and e are syntactic categories, and the compositions
%% and units are term constructors, and there is a judgement of equality of
%% e's and of alpha's, and there are definitional equality rules saying
%% A [alpha]|- B
%% alpha = beta
%% ------------------
%% A [beta]|- B
%% etc, and then the syntax of the 2-category is interpreted in an actual
%% mode 2-category.  But this seems like it would add complexity without
%% much benefit, so I went with the more semantic approach.
...
%% FIXME: explain intrinsic encoding; using derivations as proof terms
...
The following equality rules are admissible for the $U/F$ rules
described above, and for the coproduct rules in Figure~\ref{fig:coprod}.
The proofs have a lot of cases (about 800 lines of Agda total) but are
not difficult, except for somewhat subtle staging.  The rules in each of
the following groups (except the first) are proved by mutual induction,
and use the preceding groups:
...
\begin{lemma}[Some constructions on adjunctions]
...
\item We write $\semFo{e} : \arrow{}{\semFo \beta A}{\semFo \alpha A}$
  ...
  \semUo{e};\semUo{e'}$ and $\semUo{1} = 1$.  Specializing the
  definition of conjugations gives that for any $m :
...
\item We write $\semFone : \semF{1}{A} \iso A$ and $\semUone :
  ...
  $\ltor{(\semFone_A;m)}{1} = m;\inv{\semUone_B}$ (and for any $m :
  \arrow{}{A}{B}$, $\semrtol{(m;\inv{\semUone})}{1} = \semFone;m$), so in
  particular taking $m = 1$ gives $\ltor{(\semFone_A)}{{1}} =
  \inv{(\semUone_A)}$.
...
\item Because both $\M$ and $\Adj$ are strict 2-categories, and
  ...
  and $\semUcomp(1,\alpha)_A = \inv{(\semUone_{\semU{\alpha}{A}})}$.
...
The following equivalent definition of \sem{\UL{\gamma}{e}{D}} uses only
\semF{}{} operations, except for the counit 
$\semrtol{1_{\semU{\alpha}{\sem{A}}}}{\alpha} : \arrow{}{\semF{\alpha}{\semU{\alpha}{A}}}{A}$
...
However, in this case, we have restricted the class of models to
situations where the admissible rules for \ap{}{} discussed above do hold: 

\begin{theorem}[Soundness of admissible rules] 
...
\item For ${\tr{1}{D} \equiv D}$
and ${\tr{(\compv{e_1}{e_2})}{D} \equiv \tr{e_2}{\tr{e_1}{D}}}$,
