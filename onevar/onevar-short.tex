...
\institute{Wesleyan University, Middletown, CT, USA \\
           \email{dlicata@wesleyan.edu} \and
...
Benton and Wadler~\cite{benton94mixed,bentonwadler96adjoint} describe
...
metalanguage~\cite{moggi91monad} arises as $UF$.  Reed~\cite{reed09adjoint}
...
axiomatic cohesion~\cite{lawvere07cohesion}, a general categorical
...
A variation on axiomatic cohesion called \emph{cohesive homotopy type
  theory}~\cite{schreiber13dcct,schreibershulman12cohesive,shulman15realcohesion}
is currently being explored in the setting of homotopy type theory and
univalent foundations~\cite{voevodsky06homotopy,uf13hott-book}.
...
types~\cite{lumsdaine+13hits,shulman11hitsblog,lumsdaine11hitsblog}.
Theorems from homotopy theory can be proved \emph{synthetically} in this
logic~\cite{ls13pi1s1,lb13pinsn,lf14emspace,lb15cubical,favonia14covering,cavallo14mayervietoris},
and these proofs can be interpreted in a variety of
models~\cite{shulman15inversediag,voevodsky+12simpluniv,coquand+13cubical}.
...
spaces; for example, one can prove (using additional axioms) that
...
possibilities for applications to other
...
However, taking the mixed-mode judgement as primitive makes for a nicer
sequent calculus: $U$ and $F$ can be specified independently from each
other, by left and right rules, in such a way that identity and cut
(composition) are admissible, and the subformula property holds.  While
we do not consider focusing~\cite{andreoli92focus}, we conjecture that
the connectives can be given the same focusing behavior as in
\cite{reed09adjoint}: $F$ is positive and $U$ is negative (which,
because limits are negative and colimits are positive, matches what left
...
transformations~\cite[\S IV.7]{maclane98working}.  We show that the
...
theory used in \cite{shulman15realcohesion}.  All of the syntactic
metatheory of the logic and the examples have been formalized in
Agda~\cite{norell07thesis}.\footnote{See
...
another category $p$ along the adjunction $\F \alpha {} \la \U \alpha
...
\caption{Rules for coproducts; see the extended version for the
  definition of the admissible identity and cut principles, and for the
...
Figure~\ref{fig:examples}.  Though we do not consider focusing formally,
we conjecture that these two rules are invertible.
...
in Shulman~\cite{shulman15realcohesion}.
...
$\Sharp{}$ modalities of axiomatic cohesion.  One direction for future
work is to extend this adjoint logic with multiple assumptions and
dependent types (we discuss some special cases in the extended version).
...
these issues can be addressed as in \cite{shulman15realcohesion} by
...
to $\Flat{}$ (adjoint logic is still essential for representing
$\Flat{}$ and $\Sharp{}$ themselves).  Another is to consider
$\infty$-category semantics, rather than the 1-categorical semantics of
derivations that we have considered here.  A final direction for future
work is to look for applications of other mode theories in our
generalized adjoint logic beyond the motivating example of triple
adjunctions and cohesive homotopy type theory.
...
\bibliographystyle{drl-common/splncs03}
