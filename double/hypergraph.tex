...
\usepackage{amsthm,mathtools,mathpartir,mathrsfs,amssymb,url,paralist,xspace,braket,ifmtarg,cancel}
\usepackage{xcolor}
...
\fxnote{Not entirely sure about the linearity of $\xi$ here}
...
\begin{mathpar}
  ...
  \and
  \inferrule{\Gamma\types_{\alpha_n} B' \\ B'\doteq B \\ \Psi_1,B,\Psi_2 \types_{\alpha_{k+1+m}} C \\\\
    \Gamma'\doteq\Gamma \\ \Psi_1'\doteq\Psi_1 \\ \Psi_2'\doteq\Psi_2 \\ C'\doteq C}
  {\Psi'_1,\Gamma',\Psi'_2\types_{\alpha_{k+n+m}} C'}
\end{mathpar}
...
\begin{mathpar}
  ...
  \and
  \inferrule{\Gamma\types B \\ \Psi_1,B,\Psi_2 \types C}
  {\Psi_1,\Gamma,\Psi_2\types C}
\end{mathpar}
...
\begin{mathpar}
  \inferrule*{\Gamma\types A \\
    \inferrule*{\Delta,A,A\types B}{\Delta,A\types B}}{\Delta,\Gamma\types B}
  \and
  \inferrule*{\inferrule*{\Gamma\types A \\
      \inferrule*{\Gamma\types A \\ \Delta,A,A\types B}{\Delta,\Gamma\types B}}{\Delta,\Gamma,\Gamma\types B}}
  {\Delta,\Gamma\types B}
  \and
\end{mathpar}
...
\begin{mathpar}
\Delta,x:p,\Phi,\Theta \mid\cdot \mid \alpha_{n,m}(\Delta,x,x,\Phi;\Theta) \types \alpha_{n,m}(\Delta,x,\Phi;\Theta)
\and
\Delta,x:p,\Phi,\Theta \mid\cdot \mid \alpha_{n,m}(\Delta;\Theta,x,x,\Phi) \types \alpha_{n,m}(\Delta;\Theta,x,\Phi)
\and
\Delta,x:p,\Phi,\Theta \mid\cdot \mid \alpha_{n,m}(\Delta,\Phi;\Theta) \types \alpha_{n,m}(\Delta,x,\Phi;\Theta)
\and
\Delta,x:p,\Phi,\Theta \mid\cdot \mid \alpha_{n,m}(\Delta;\Theta,\Phi) \types \alpha_{n,m}(\Delta;\Theta,x,\Phi)
\end{mathpar}
generating (after reduction) the contraction and weakening rules:
\begin{mathpar}
\inferrule{\Gamma,A,A,\Psi \types \Theta}{\Gamma,A,\Psi \types \Theta}
\and
\inferrule{\Gamma \types \Theta,A,A,\Psi}{\Gamma \types \Theta,A,\Psi}
\and
\inferrule{\Gamma,\Psi \types \Theta}{\Gamma,A,\Psi \types \Theta}
\and
\inferrule{\Gamma \types \Theta,\Psi}{\Gamma \types \Theta,A,\Psi}
\end{mathpar}
...
\begin{mathpar}
  ...
  \cdot \Types Y:p \and
  \cdot \Types Z:p \and
  \cdot \mid\cdot \mid\cdot \types f:\alpha(X,Y;Z) \and
  \cdot \mid\cdot \mid\cdot \types g:\alpha(Y,Z,Y;X) \and
  \cdot \mid\cdot \mid\cdot \types h:\alpha(;Y) \and
\end{mathpar}
or equivalently premise-free rules, i.e.\ axioms:
\begin{mathpar}
  \inferrule{ }{X\type_p}\and
  \inferrule{ }{Y\type_p}\and
  \inferrule{ }{Z\type_p}\\
  \inferrule{ }{X,Y\types f:Z}\and
  \inferrule{ }{Y,Z,Y\types g:X}\and
  \inferrule{ }{\types h:Y}\and
\end{mathpar}
...
\begin{mathpar}
  x:p,y:p,z:p \mid \cdot \mid \alpha(x;y),\alpha(y;z) \types \alpha(x;z)
  \and
  x:p \mid\cdot\mid \cdot \types \alpha(x;x)
\end{mathpar}
...
Adding a \fxnote*{which we should discuss in general somewhere}{``representability'' criterion} we obtain ``double generalized multicategories'', and if the vertical arrows are trivial we obtain ``generalized 2-multicategories''.
...
We assume that the virtual hypercategory is strong (\cref{thm:strong})\fxerror{Need to generalize away from that.}
...
\begin{defn}
  In a weakly directed\fxnote{and strong?} virtual hypercategory, suppose $\omega : (p_1,\dots,p_n)\to q$ is an arrow and $\alpha(p_1,\dots,p_n,q)$ is a morphism, and suppose that $q$ has a unit as in \cref{defn:unit}.
  We say $\alpha$ and $\omega$ are \textbf{companions} if we have 2-cells
  \[ x_1:p_1,\dots,x_n:p_n,y:q \mid\cdot \mid \alpha(x_1,\dots,x_n,y) \types \iota_q(\omega(x_1,\dots,x_n);y) \]
  \[ \hspace{-2cm} x_1:p_1,\dots,x_n:p_n \mid\cdot\mid \cdot \types \alpha(x_1,\dots,x_n, \omega(x_1,\dots,x_n)) \hspace{-2cm}\]
  or equivalently
  \begin{mathpar}
    \inferrule{\types_\alpha A_1,\dots,A_n,B}{\form\omega(A_1,\dots,A_n) \types_{\iota_q} B}
    \and
    \inferrule{ }{\types_\alpha A_1,\dots,A_n,\form\omega(A_1,\dots,A_n)}
  \end{mathpar}
  that are ``inverses in both directions''.
  In one direction this means that
  \begin{equation*}
    \inferrule*{
      \inferrule*{ }{\types_\alpha A_1,\dots,A_n,\form\omega(A_1,\dots,A_n)}
    }{\form\omega(A_1,\dots,A_n) \types_{\iota_q} \form\omega(A_1,\dots,A_n)}
    \quad=\quad
    \inferrule*{ }{\form\omega(A_1,\dots,A_n) \types_{\iota_q} \form\omega(A_1,\dots,A_n)}
  \end{equation*}
  ...
  \begin{mathpar}
    \inferrule*{
      ...
      \\
      \inferrule*{\types_\alpha A_1,\dots,A_n,B}{\form\omega(A_1,\dots,A_n) \types_{\iota_q} B}
    }{\types_\alpha A_1,\dots,A_n,B}
  \end{mathpar}
  is the identity (the cut rule on the bottom is induced by the universal property of the unit).
\end{defn}

For simplicity we have written the definition in the case that $q$ is the last entry of $\alpha$, but it is clearly possible to give an analogous definition wherever it appears, and however $\alpha$ is directed, and even if the $p_i$'s are not in the same order in $\alpha$ and $\omega$.
In the strongly directed case, however, the $p_i$'s must be a inputs of $\alpha$ and $q$ an output, in order for the first 2-cell to be well-typed.

We noted in \cref{sec:gen-2-multi} that virtual double categories can be viewed as particular virtual hypercategories.
In this case, the above notion of companion reduces to the usual one from (virtual) double category theory (see for instance~\cite{cs:multicats}).

In terms of the object-language, however, the companion 2-cells express the left and right sequent calculus rules (without built-in cuts) for a type former $\form\omega$ that ``internalizes'' a mode morphism $\alpha$ on the left.
For instance, when $\alpha = \alpha_2(p,p;p)$ in a multicategory (where $\iota = \alpha_1$), and $q$ is the last entry of $\alpha_2$, the rules become
\begin{mathpar}
  \inferrule{A_1,A_2 \types B}{\form\omega(A_1,A_2) \types B}
  \and
  \inferrule{ }{A_1,A_2 \types \form\omega(A_1,A_2)}
\end{mathpar}
so that $\form\omega(A_1,A_2)$ is a tensor product $A_1\otimes A_2$.
Similarly, in adjoint logic this yields the $F$-functor associated to a mode morphism.

The dual notion of a companion is the following:

\begin{defn}
  In a weakly directed\fxnote{and strong?} virtual hypercategory, suppose $\omega : (p_1,\dots,p_n)\to q$ is an arrow and $\alpha(p_1,\dots,p_n,q)$ is a morphism, and suppose that $q$ has a unit as in \cref{defn:unit}.
  We say $\alpha$ and $\omega$ are \textbf{conjoints} if we have 2-cells
  \[ x_1:p_1,\dots,x_n:p_n,y:q \mid\cdot \mid \alpha(x_1,\dots,x_n,y) \types \iota_q(y;\omega(x_1,\dots,x_n)) \]
  \[ \hspace{-2cm} x_1:p_1,\dots,x_n:p_n \mid\cdot\mid \cdot \types \alpha(x_1,\dots,x_n,\omega(x_1,\dots,x_n)) \hspace{-2cm}\]
  or equivalently
  \begin{mathpar}
    \inferrule{\types_\alpha A_1,\dots,A_n,B}{B\types_{\iota_q}\form\omega(A_1,\dots,A_n) }
    \and
    \inferrule{ }{\types_\alpha A_1,\dots,A_n,\form\omega(A_1,\dots,A_n)}
  \end{mathpar}
  such that analogous equations hold.
\end{defn}

For instance, when $\alpha=\alpha_2(p,p;p)$ in a multicategory, and $q$ is the first $p$ occurring in $\alpha_2$, these rules become
\begin{mathpar}
  \inferrule{B,A_1 \types A_2}{B \types \form\omega(A_1,A_2)}
  \and
  \inferrule{ }{\form\omega(A_1,A_2),A_1\types A_2}
\end{mathpar}
so that $\form\omega(A_1,A_2)$ is a function-type $A_1\multimap A_2$.
Similarly, in adjoint logic this yields the $U$-functor associated to a mode morphism.

Note that the rules for function-types are not strongly directed, so we cannot assume strong directedness in general.
It does seem sensible to assume for a companion that $q$ is \emph{among} the outputs of $\alpha$, and for a conjoint that $q$ is \emph{among} the inputs; but the definitions don't seem to require that.
...
Then the rules are
\begin{mathpar}
  \inferrule{\types A,B}{\form\omega(A)\types B}
  \and
  \inferrule{ }{\types A, \form\omega(A)}
\end{mathpar}
