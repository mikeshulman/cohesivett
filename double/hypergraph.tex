...
\end{mathpar}
generating the contraction and weakening rules:
\begin{mathpar}
...
In an undirected mode theory with one mode $p$ and one morphism $\alpha_n(\overbrace{p,\dots,p}^n)$, this can be generated by 2-cells
\[ \Delta,x:p \mid\cdot\mid \alpha_{n+1}(\Gamma,x), \alpha_{m+1}(\Phi,x) \types \alpha_{n+m}(\Gamma,\Phi) \]
followed by a canonical reduction from
\[\inferrule{\types \Gamma,A \\ A\doteq B \\ \types \Phi,B}{\types \Gamma,\Phi} \]
The corresponding identity 2-cell is
\[ x:p \mid\cdot \mid \cdot \types \alpha_2(x,x) \]
yielding the rule
\[ \inferrule{A\type}{\types A,A} \]
One usually includes an exchange rule too, which can be done in the obvious way.
...
The coherence theorem for cyclic multicategories involves arbitrary undirected trees.
