...
\begin{align}
\label{mor-fib-subst}
f_1(\beta,\TrPlus{\ap{S}{s/x}}{\mu}) \equiv_{T(x)} \TrPlus{\ap{T}{s/x}}{f_1(\alpha,\mu)}\\
\label{mor-opfib-subst}
f_1(\alpha,\TrCirc{\ap{S}{s/x}}{\nu}) \equiv_{T(x)} \TrCirc{\ap{T}{s/x}}{f_1(\beta,\nu)}
\end{align}
...
\subsection{Fibred Terminal Object}

\begin{definition}
For a dependent mode $x : p \vdash S(x) \type$, a \emph{fibred terminal object} is a term
\begin{mathpar}
x : p \vdash 1_x : S(x)
\end{mathpar}
such that
\begin{align}
\label{bang-unique}
t & \equiv \bang && \text{where } \TermTwoT{\gamma}{t}{\mu}{1_\alpha}{S(\alpha)}\\
...
\label{one-terminal} 
1_\alpha &\text{ is a fibred terminal object of } \El{p}{\alpha} \\
\label{circ-one}
...
The triangle equalities for $\chi^\circ \dashv \chi^+$ take the following form:
\begin{align*}
...
\end{align*}
because $1_\alpha$ is terminal, the latter is just saying $\eta_\alpha ; \pi^\alpha_{1_\alpha} = \id_\alpha$.
...
\begin{definition}
  A comprehension object is \mvrnote{super cool} if additionally:
\begin{align}
\label{chi-monad-trivial}
...
%\begin{align*}
%\id_{\alpha.{1_\alpha}}
%&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\chi}{z})}{(\pi_1^\alpha, \var{1})/z} \\
%&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\chi}{z})}{(\pi_1^\alpha, \ap{1_w}{\pi_1^\alpha/w})/z} \\
%&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\chi}{z})}{(\pi_1^\alpha, \ap{1_w}{\pi_1^\alpha/w})/z} \\
%&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\chi}{\TrCirc{\chi}{z}})}{\pi_1^\alpha/z} \\
