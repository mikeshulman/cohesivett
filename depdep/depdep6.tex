...
\end{align*}
With corresponding term equalities: \mvrnote{TODO: F and U and telescope equations}
\begin{align*}
...
%\end{lemma}
%\begin{proof}
%\mvrnote{Todo:} For telescope types:
%\begin{align*}
%(\rewrite{s}{M},N) 
%&\equiv  \\
%(M,\rewrite{t}{N}) 
...
\begin{mathpar}
\rewrite{t}{\UStI{s}{M}} \equiv \UStI{s}{\rewrite{\ap{s^\circ}{t}}{M}} \and \rewrite{\ap{s^\circ}{t}}{\UnSt{s}{M}} \equiv \UnSt{s}{\rewrite{t}{M}}
\end{mathpar}
\end{lemma}
\begin{proof}
\mvrnote{easy}
...
\begin{lemma}
Any context $\Gamma$ can be tupled into a $\Sigma$ type $\Sigma \Gamma$, so that substitutions $\Gamma \yields \Theta : \Delta$ correspond bijectively to terms $\Gamma \yields \Sigma \Theta : \Sigma \Delta$. \mvrnote{Generalising to telescopes seems straightforward}
\begin{mathpar}
\inferrule*{\yields_\gamma \Gamma}
             {\cdot \yields_{\Sigma \gamma} \Sigma \Gamma \TYPE}
\and
\inferrule*{~}
             {\sigma : \Sigma \Gamma \yields_{\proj{\gamma}} \proj{\Gamma} : \Gamma}
\and
...
\inferrule*{\Gamma \yields\Theta : \Delta}
             {\Gamma \yields \Theta \equiv \proj{\Delta}[\Sigma \Theta / \sigma]}
\end{mathpar}
\end{lemma}
\begin{proof}
...
\begin{align*}
\Sigma (\cdot) &:\equiv 1 \\
\Sigma (\Gamma, x : A) &:\equiv \sum_{\sigma : \Sigma \Gamma} A[\proj{\Gamma}] \\
\proj{(\cdot)} &:\equiv \cdot \\
\proj{\Gamma, x : A} &:\equiv (\proj\Gamma[\fst{\sigma}/\sigma]), \snd{\sigma}
\end{align*}
with each definition lying over the identical one downstairs. For $\Sigma \Theta$ we simultaneously verify the equation $\Theta \equiv \proj{\Delta}[\Sigma \Theta / \sigma]$, as we need it to hold for $(\Sigma \Theta, M)$ to be well typed.
\begin{align*}
\Sigma(\cdot) &:\equiv \mt : 1\\
\Sigma(\Theta, M : A) &:\equiv (\Sigma \Theta, M) : \sum_{\sigma : \Sigma \Delta} A[\proj{\Delta}] \\
(\cdot)[()/\sigma]  &\equiv (\cdot) \\
\proj{\Delta, x : A}[\Sigma(\Theta, M : A)/\sigma] &\equiv ((\proj{\Delta}[\fst{\sigma}/\sigma]), \snd{\sigma})[(\Sigma \Theta, M)/\sigma] \\
&\equiv (\proj{\Delta}[\Sigma \Theta/\sigma]), M \\
&\equiv \Theta, M
\end{align*}
\end{proof}
...
\end{definition}
In particular, if $\gamma\yields t : ((\theta, \mu) \vDash (\theta', \mu')) : (\delta, x : p)$ then
\begin{align*}
\gamma &\yields \ap{\fst}{t} : \theta \vDash \theta' \\
\gamma &\yields \ap{\snd}{t} : (\mu[\theta] \vDash \TrPlus{(\ap{p[\proj{\delta}]}{\Sigma t/\sigma})}{\mu'[\theta']})
\end{align*}

\begin{lemma}
N-ary ap is admissible:
\begin{mathpar}
...
These are
\begin{align*}
\ap {q} {t} &:\equiv \ap{q[\proj{\delta}]}{\Sigma t/\sigma} \\
\ap {\nu} {t} &:\equiv \ap{\nu[\proj{\delta}]}{\Sigma t/\sigma}
\end{align*}
which are well-typed by the equation $\theta \equiv \proj{\delta}[\Sigma \theta / \sigma]$.
\end{proof}

\begin{lemma}
N-ary rewrite is admissible:
\begin{mathpar}
 \inferrule*[Left = rewrite]{
...
This is
\begin{align*}
\rewrite{t}{\Theta} &:\equiv \proj{\Delta}[\rewrite{\Sigma t}{\Sigma \Theta}/\sigma]
\end{align*}
\end{proof}
...
\begin{align*}
B[\rewrite{t}{\Theta}] 
&\equiv B[\proj{\Delta}[\rewrite{\Sigma t}{\Sigma \Theta}/\sigma]] \\
&\equiv B[\proj{\Delta}][\rewrite{\Sigma t}{\Sigma \Theta}/\sigma] \\
&\equiv \St{\ap{q[\proj{\delta}]}{\Sigma t / \sigma}}{B[\proj{\Delta}][\Sigma \Theta/\sigma]} \\
&\equiv \St{\ap{q}{t}}{B[\Theta]}
\end{align*}
And:
\begin{align*}
N[\rewrite{t}{\Theta}]
&\equiv N[\proj{\Delta}[\rewrite{\Sigma t}{\Sigma \Theta}/\sigma]] \\
&\equiv N[\proj{\Delta}][\rewrite{\Sigma t}{\Sigma \Theta}/\sigma] \\
&\equiv \rewrite{\ap{\nu[\proj{\delta}]}{\Sigma t/\sigma}}{\StI{\ap{q[\proj{\delta}]}{\Sigma t/\sigma}}{N[\proj{\Delta}][\Sigma \Theta/\sigma]}} \\
...
%&\equiv \ap{(\ApEl{p}{\pi^\alpha_{\TrCirc{\ApEl{p}{s}}{1_\beta}}})^\circ}{\ap{(\ApEl{p}{\pi^\alpha_{\TrCirc{\ApEl{p}{s}}{1_\beta}}})^+}{?}} ;\varepsilon^{\ApEl{p}{\pi^\alpha_{\TrCirc{\ApEl{p}{s}}{1_\beta}}}}_{\TrCirc{\ApEl{p}{s}}{1_\beta}} \\
%&\equiv \varepsilon^{\ApEl{p}{\pi^\alpha_{\TrCirc{\ApEl{p}{s}}{1_\beta}}}}_{?} ; ?  \\
&\equiv ??? \\
...
%\begin{align*}
%\eta_{\TrCirc{\ApEl{p}{H(s)}}{1_\beta}} \equiv \varepsilon^{\ApEl{p}{H(s)}}_{1_{\alpha.\TrCirc{\ApEl{p}{s}}{1_\beta}}}; \var{\TrCirc{\ApEl{p}{s}}{1_\beta}}
%\end{align*}
%Starting with the RHS:
%\begin{align*}
%\varepsilon^{\ApEl{p}{H(s)}}_{1_{\alpha.\TrCirc{\ApEl{p}{s}}{1_\beta}}}; \var{\TrCirc{\ApEl{p}{s}}{1_\beta}}
%&\equiv \varepsilon^{\ApEl{p}{\eta^\chi_\beta};\ApEl{p}{(s \bdot \eta^{\ApEl{p}{s}}_{1_\beta})}}_{1_{\alpha.\TrCirc{\ApEl{p}{s}}{1_\beta}}}; \var{\TrCirc{\ApEl{p}{s}}{1_\beta}} \\
%&\equiv \varepsilon^{\ApEl{p}{\eta^\chi_\beta};\ApEl{p}{(s \bdot \eta^{\ApEl{p}{s}}_{1_\beta})}}_{1_{\alpha.\TrCirc{\ApEl{p}{s}}{1_\beta}}}; \var{\TrCirc{\ApEl{p}{s}}{1_\beta}} \\
%&(\ap{{\ApEl{p}{(s \bdot \eta^{\ApEl{p}{s}}_{1_\beta})}}^\circ}{\varepsilon^{\ApEl{p}{\eta^\chi_\beta}}_{{\ApEl{p}{(s \bdot \eta^{\ApEl{p}{s}}_{1_\beta})}}^+(1_{\alpha.\TrCirc{\ApEl{p}{s}}{1_\beta}})}}) ; \varepsilon^{\ApEl{p}{(s \bdot \eta^{\ApEl{p}{s}}_{1_\beta})}}_{1_{\alpha.\TrCirc{\ApEl{p}{s}}{1_\beta}}}; \var{\TrCirc{\ApEl{p}{s}}{1_\beta}} \\
%\end{align*}
\end{proof}
...
\inferrule*[Left=s-intro-2] {
\inferrule*{
 \upstairs{\Gamma}, x : \upstairs{A} \yields_x x : \upstairs{A}[\upstairs{\proj{\Gamma, A}}] \and E(x) : \TrCirc{\pi^{\asdep{\Gamma}}_x}{1_{\asdep{\Gamma}.x}} \equiv x
 }{
 \upstairs{\Gamma}, x : \upstairs{A} \yields_{\TrCirc{\pi^{\asdep{\Gamma}}_x}{1_{\asdep{\Gamma}.x}}} x : \upstairs{A}
}}{
\upstairs{\Gamma}, x : \upstairs{A} \yields_{1_{\asdep{\Gamma}.x}} \UStI{\pi^{\asdep{\Gamma}}_x}{x} : \St{\pi^{\asdep{\Gamma}}_x}{\upstairs{A}}
}
\end{mathpar}
\end{enumerate}
...
\item The derived substitution $\hat{a}$ is \mvrnote{todo}.
\end{itemize}
...
\subsubsection{Translation to a Weak Comprehension Object}

Now we see if some of the equations can be omitted.
...
% $T : \downstairs{\Theta} \vDash \downstairs{\Theta'}$ where $\downstairs{\Theta}, \downstairs{\Theta'} : \downstairs{\Gamma} \vdash \downstairs{\Delta}$ . Such a morphism is defined inductively as either the unique morphism between empty substitutions, or as a pair $(T, t) : (\downstairs{\Theta}, \mu) \vDash (\downstairs{\Theta'}, \mu')$ where
%\begin{align*}
%T : \downstairs{\Theta} &\vDash \downstairs{\Theta'} & :\downstairs{\Gamma} \yields \downstairs{\Delta} \\
%t : \mu &\vDash_{\downstairs{A}[\downstairs{\Theta}]} \TrPlus{\ap{\downstairs{A}}{T}}{\mu'}
%\end{align*}
%and the operation $\downstairs{\Gamma} \yields \ap{\downstairs{B}}{T} : \downstairs{B}[\downstairs{\Theta}] \vDash \downstairs{B}[\downstairs{\Theta'}]$ is defined inductively by:
%\begin{align*}
%\ap{\downstairs{B}}{\cdot} &:\equiv \id_{\downstairs{B}} \\
%\ap{\downstairs{B}}{T, t} &:\equiv \ap{\downstairs{B}[\downstairs{\Theta}]}{t/x};?
%\end{align*}
%This is what $\Sigma$-types were supposed to do.
