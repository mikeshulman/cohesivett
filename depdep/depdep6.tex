...
\\
t;\eta_\mu \equiv \eta_{\mu'} ; \ap{\TrPlus{s}{\TrCirc{s}{z}}}{t/z} \and \varepsilon_\nu;t \equiv \ap{\TrCirc{s}{\TrPlus{s}{z}}}{t/z} ; \varepsilon_{\nu'}
\end{mathpar}
\msnote{Shouldn't those be
$\TermTwoT{\gamma}{\varepsilon_\nu}{\TrCirc{s}{\TrPlus{s}{\nu}}}{\nu}{q}$ and
$\TermTwoT{\gamma}{\eta_\mu}{\mu}{\TrPlus{s}{\TrCirc{s}{\mu}}}{p}$?
}
...
\msnote{Shouldn't that be $\TermTwoT{\gamma}{s}{\nu}{\mu}{p}$ in the 2cell rule, where $\Gamma \yields_p A \TYPE$?}
...
\label{beta-var}
\ap{\ApEl{p}{(s \bdot m)}^+}{\var{\mu}} &\equiv m && \text{where } \TermTwoT{\gamma}{m}{1}{\ApEl{p}{s}^+(\mu)}{\El{p}{\beta}} \\
\label{eta-pi-var}
t &\equiv ((t;\pi) \bdot \ap{\ApEl{p}{t}^+}{\var{\mu}}) && \text{where } \TermTwoT{\gamma}{t}{\beta}{\alpha.\mu}{p} \\
\label{dot-circ}
\alpha. (\ApEl{p}{s}^\circ(\nu)) &\equiv \beta.\nu && \\
\label{pi-dot-circ}
...
\item Projection of the last ``variable'' from a ``context'' is defined
  ...
  \ApEl{p}{\pi}^+$:
\begin{mathpar}
  {\TermTwoT{\alpha:p,x:\El{p}{\alpha}}{\var{x}}{1_{\alpha.x}}{\ApEl{p}{\pi}^+(x)}{\El{p}{\alpha.x}}} 
    \and
...
\subsection{Weak Comprehension Object (from mode type morphism)}

\newcommand{\rdot}{\mathrm{dot}}

\begin{definition}[Weak Comprehension Object]
  A \emph{weak comprehension object} is specified by the following
  constants:
  \begin{mathpar}
    p \type \and
    \emptyset : p %\and
%    \TermTwoT{\alpha : p}{\bang_p}{\alpha}{\emptyset}{p}
    \\
    \alpha : p \yields \El{p}{\alpha} \type \and
    \TypeTwo{}{\rdot}{p}{\sigmacl{\alpha}{p}{\El{p}{\alpha}}}
  \end{mathpar}
  such that
  \begin{align}
    \label{rdot-section}
    \fst{\TrCirc{\rdot}{\alpha}} \equiv \alpha
  \end{align}
  where:
  \begin{itemize}
  \item Comprehension is given by $\rdot^+$:
  \begin{mathpar}
  \alpha : p, x : \El{p}{\alpha} \yields \alpha.x : p \and \alpha.x :\equiv \TrPlus{\rdot}{(\alpha, x)}
  \end{mathpar}
%  \item Pairing for the comprehension object is given by ap of
%  $.$ on the pairing for $\Sigma$-modes:
%  \begin{mathpar}
...
%  s \bdot m :\equiv \ap{(\fst z \bdot \snd z)}{(s,t)/ (z:\sigmacl{\alpha}{p}{\El{p}{\alpha}})}
%  \end{mathpar}
  \item The unit term in $\El{p}{\alpha}$ is given by 
  \begin{mathpar}
  \alpha : p \yields 1_\alpha : \El{p}{\alpha} \and 1_\alpha :\equiv \snd{\TrCirc{\rdot}{\alpha}}
  \end{mathpar}
  This term is in $\El{p}{\fst{\TrCirc{\rdot}{\alpha}}} \equiv \El{p}{\alpha}$ by equation \eqref{rdot-section}.
  \item The counit of $\rdot$ gives us both $\pi^\alpha_\mu$ and $\var{\mu}$ directly:
  \begin{mathpar}
  {\TermTwoT{\alpha:p,x:\El{p}{\alpha}}{\pi^\alpha_x}{\alpha.x}{\alpha}{p}}
  \and
  \pi^\alpha_x :\equiv \ap \fst {\varepsilon_{(\alpha, x)}} \\
  {\TermTwoT{\alpha:p,x:\El{p}{\alpha}}{\var{x}}{1_{\alpha.x}}{\ApEl{p}{\pi^\alpha_x}^+(x)}{\El{p}{\alpha.x}}} 
    \and
    \mathsf{var}_x :\equiv \ap \snd {\varepsilon_{(\alpha, x)}}
  \end{mathpar}
  \end{itemize}
\end{definition}

So in all, the unit and counit of $\rdot$ are maps
\begin{align*}
&\TermTwoT{}{\eta}{\alpha}{\alpha.{1_\alpha}}{p} \\
&\TermTwoT{}{(\pi_x^\alpha, \var{x})}{(\alpha.x, 1_{\alpha.x})}{(\alpha, x)}{\sigmacl{\alpha}{p}{\El{p}{\alpha}}}
\end{align*}
The first triangle equation is
\begin{mathpar}
\eta_{\TrPlus{\rdot}{(\alpha, x)}};\ap{(\TrPlus{\rdot}{z})}{\varepsilon_{(\alpha, x)}/z} \equiv \id_{\TrPlus{\rdot}{(\alpha, x)}} 
\end{mathpar}
And using some of the above definitions:
\begin{mathpar}
\eta_{\alpha.x};\ap{(\TrPlus{\rdot}{z})}{(\pi_x^\alpha, \var{x})/z} \equiv \id_{\alpha.x}
\end{mathpar}

The second triangle equation
\begin{mathpar}
\ap{(\TrCirc{\rdot}{z})}{\eta_\alpha/z};\varepsilon_{\TrCirc{\rdot}{\alpha}} \equiv \id_{\TrCirc{\rdot}{\alpha}}
\end{mathpar}
On the left this is:
\begin{align*}
\ap{(\TrCirc{\rdot}{z})}{\eta_\alpha/z};\varepsilon_{\TrCirc{\rdot}{\alpha}}
...
\end{align*}
We are also hoping that $\pi_{1_\alpha}^\alpha ; \eta_\alpha \equiv 1_{\alpha.1}$. This seems to follow if we know that $\ap{1_w}{\pi_1^\alpha/w} \equiv \var{1_\alpha}$, because then we can use the first triangle equation at $x = 1_\alpha$:
\begin{align*}
\id_{1_\alpha} 
&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\rdot}{z})}{(\pi_1^\alpha, \var{1})/z} \\
&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\rdot}{z})}{(\pi_1^\alpha, \ap{1_w}{\pi_1^\alpha/w})/z} \\
&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\rdot}{z})}{(\pi_1^\alpha, \ap{1_w}{\pi_1^\alpha/w})/z} \\
&\equiv \eta_{\alpha.1};\ap{(\TrPlus{\rdot}{\TrCirc{\rdot}{z}})}{\pi_1^\alpha/z} \\
&\equiv \pi_1^\alpha;\eta_{\alpha}
\end{align*}
where that last move is naturality of $\eta$. That condition holds if $1_\alpha$ is terminal.
