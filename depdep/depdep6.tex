...
\begin{mathpar}
  ...
  }{\Gamma \yields_r \U{c.\mu}{A} \TYPE \and (\text{where } \gamma \yields r \type)} \\
...
: S[\alpha/x] \to S[\alpha/x]$ and $\ap{S}{s/x}^\circ : S[\alpha/x] \to
S[\alpha/x]$.
...
q \type \and y : q \vdash T(x) \type 
\end{mathpar}
A \emph{morphism of bifibrations} (\drlnote{should this be called
  something more specific? does ``bifibration'' suggest the adjunction
  preservation?}) from $(p,S)$ to $(q,T)$ consists of terms
\begin{mathpar}
...
is a ``morphism of opfibrations'' in a dual sense.  The third and fourth
say that $f_1$ preserves the unit and counit of the $s^\circ \dashv s^+$
adjunctions.  To type check \eqref{mor-fib-unit}, we have
\begin{mathpar}
...
\begin{mathpar}
\TrCirc{t[\alpha/x]}{\TrCirc{\ap{S}{s/x}}{\nu}} \equiv \TrCirc{\ap{T[f_0(x)/y]}{s/x}}{\TrCirc{\beta}{\nu}}\\
\end{mathpar}
In such circumstances, asserting that $(f_0,f_1)$ is a morphism of
bifibrations from $(p,S)$ to $(q,T)$ means assertin equations
\eqref{mor-fib-subst} \eqref{mor-fib-unit} \eqref{mor-fib-counit}.

\subsection{Strict Internal-Comprehension-Object (Axiomatic, $s^\circ$ and $s^+$ primitive)}

\begin{definition}[Internal Comprehension Object]
  A \emph{strict internal comprehension object} is specified by the following
  constants:
  \begin{mathpar}
    p \type \and
    \emptyset : p \and
...
\yields \mu : \alpha$ and $\gamma \yields \nu : \beta$ unless otherwise specified)
\begin{align}
...
\label{beta-var}
\ap{\ApEl{p}{(s \bdot m)}^+}{\mathsf{var}_\mu} &\equiv m && \text{where } \TermTwoT{\gamma}{m}{1}{\El{p}{s}^+(\mu)}{\El{p}{\beta}} \\
\label{eta-pi-var}
...
In the equations, we use the following derived forms:
\begin{itemize}
\item Pairing for the internal comprehension object is given by ap of
  $.$ on the pairing for $\Sigma$-modes:
  \begin{mathpar}
  \inferrule{\TermTwoT{\gamma}{s}{\alpha}{\beta}{p} \and
             %% \and \gamma \yields \mu : \alpha \and \gamma \yields \nu : \beta \and
             \TermTwoT{\gamma}{m}{\mu}{\El{p}{s}^+(\nu)}{\El{p}{\alpha}}}
            {\TermTwoT{\gamma}{s \bdot m}{\alpha.\mu}{\beta.\nu}{p}} \and
  s \bdot m :\equiv \ap{(\fst z \bdot \snd z)}{(s,t)/ (z:\sigmacl{\alpha}{p}{\El{p}{\alpha}})}
  \end{mathpar}
  
  By \eqref{dot-one-strict}, this has the usual comprehsion operation on
  $p$ as a special case, if we take the terms to be maps
  \TermTwoT{\gamma}{m}{1}{\El{p}{s}^+(\nu)}{\El{p}{\alpha}}
  in the fiber:
  \begin{mathpar}
  \inferrule{\TermTwoT{\gamma}{s}{\alpha}{\beta}{p} \and
             %% \and \gamma \yields \mu : \alpha \and \gamma \yields \nu : \beta \and
...
\begin{mathpar}
    ...
    {\TermTwoT{\gamma}{\tilde{s}}{\alpha.\El{p}{s}^+(\mu)}{\beta.\mu}{p}}
    \and
    \tilde{s} :\equiv (s \bdot \id_{\El{p}{s}^+(\mu)}) 
\end{mathpar}
...
\item Projection of the last ``variable'' from a ``context'' is defined
  using the unit of the adjunction $\El{p}{\pi}^\circ \dashv
  \El{p}{\pi}^+$:
  
\begin{mathpar}
  {\TermTwoT{\alpha:p,x:\El{p}{\alpha}}{\mathsf{var}_x}{1_{\alpha.x}}{\El{p}{\pi}^+(x)}{\El{p}{\alpha.x}}} 
    \and
    \mathsf{var}_x :\equiv \eta_{\pi^\alpha_x}(1_{\alpha.x}) \\
  \end{mathpar}
...
To type check the equations, we use the following derivable equations:
\begin{itemize}

\item Fusion for $.$
\begin{align}
\label{dot-fusion}
    (s \bdot m);(s' \bdot m') \equiv ((s;s') \bdot (m;\ap {\El{p}{s}^+} {m'}))
\end{align}
...
\item $\beta$ reduction for first projection:
  \begin{align}
\label{beta-pi}
(s \bdot m);\pi &\equiv s && \text{where } \TermTwoT{\gamma}{m}{1_\alpha}{\El{p}{s}^+(\mu)}{\alpha}
  \end{align}
  is derived by
  \begin{mathpar}
...
$\alpha \Yields \beta$.  Equation \eqref{bang-unique} is the analogue of
uniqueness of terms of unit type.  Equation \eqref{circ-one} is an
$\eta$ principle that a term of mode $\El{p}{\alpha}$ (i.e. a
``dependent type'') is determined by its comprehension and projection.
Equation \eqref{beta-var} says that ``substituting'' into a ``variable''
is second projection on the ``substitution''; it type checks using
\eqref{s-plus-one-strict} and the derivable corresponding equation for
first projection \eqref{beta-pi} (and fusing the transports/aps).
Equation \eqref{eta-pi-var} is the usual $\eta$ principle for
subsitutions.  Equation \eqref{dot-circ} says that the comprehension of
$s^\circ(\mu)$ is the same as the comprehension of $\mu$, while equation
\eqref{pi-dot-circ} says that the projection is determined by $s$ and
the projection from $\beta$ (which type checks using \eqref{dot-circ}).
Finally, we say that $\El{p}{\pi}^\circ$ is a morphism
of bifibrations in the above sense.  

%% where $\eta_s$ and $\varepsilon_s$ are the unit and counit for the adjunction $\El{p}{s}^\circ \dashv \El{p}{s}^+$.

Some other equations that you might ask for that are already derivable are:
\begin{itemize}
