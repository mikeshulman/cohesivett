...
Idempotence of $f_0$ implies idempotence of $\flat_1$:
\begin{align*}
f_0(\alpha).\flat_1(\flat_1(x)) 
&\equiv f_0(f_0(\alpha)).\flat_1(\flat_1(x))  \\
&\equiv f_0(f_0(\alpha).\flat_1(x)) \\
&\equiv f_0(f_0(\alpha.x))\\
&\equiv f_0(\alpha.x) \\
&\equiv f_0(\alpha).\flat_1(x)
\end{align*}
...
\end{align*}
This typechecks due to equations \eqref{dot-circ} and \eqref{pi-dot-circ}. Let $\pi_{\Gamma'}$ denote the term 2-cell
\begin{align*}
...
\inferrule*[vcenter, Right = weaken-suffix]
...
\[\Gamma, x : A \yields_{1_{\underline{(\Delta ; \cdot)}.x}} \mathsf{var}_x(\StI{x}) : \pi^*(A)\]
...
\end{align*}
In crisp-$\flat$-elim, we must use idempotence to show $1_{\underline{(\Delta ; \cdot)}.\flat_1(y)}[\flat_1(\alpha,x) / y] \equiv 1_{\underline{(\Delta ; \cdot)}.\flat_1(\alpha,x)}$. TODO: add gamma to N and conclusion
