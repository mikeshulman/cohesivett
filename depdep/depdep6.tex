...
\end{align*}
With corresponding term equalities: \mvrnote{The F and U equations are a bit silly without annotations}
\begin{align*}
\StI{s}{\FI{M}} &\equiv \FI{M} \\
\FI{\StI{s}{M}} &\equiv \FI{M} \\
\StI{s}{\UI{x}{M}} &\equiv \UI{x}{M} \\
\UI{x}{\StI{s}{M}} &\equiv \UI{x}{M} \\
\UI{x}{M} &\equiv \UI{x}{M[\StI{s}{x}/x]} \\
...
\begin{align}
\label{beta-var}
\ApPlus{\ApEl{p}{(s \bdot m)}}{\var{\mu}} &\equiv \ApPlus{\ApEl{p}{\pi^\beta_{1_\beta}}}{m}  && \text{where } \TermTwoT{\gamma}{m}{1}{\TrPlus{\ApEl{p}{s}}{\mu}}{\El{p}{\beta}}
\end{align}
is derivable by:
\begin{align*}
...
&\equiv \var{1_\beta}; \ApPlus{\ApEl{p}{\pi^\beta_{1_\beta}}}{m} \\
&\equiv \ApPlus{\ApEl{p}{\pi^\beta_{1_\beta}}}{m} \\
\end{align*}
...
%\end{align}
\caption{Rules for $\Sigma$-types in MLTT via Explicit Substitutions}\label{fig:qit-weak-sigma-rules}
\end{figure}

Suppose $p$ supports weak $\Sigma$-types. In the framework, $\Sigma$-types are the $\mathsf{F}$-types for the mode term
\begin{align*}
...
{\upstairs{\Gamma}, x : \upstairs{A}, y : \upstairs{B} \yields_{1_{\asdep{\Gamma}.x.y}} \rewrite{\var{y};\ApPlus{\pi^{\asdep{\Gamma}.x}_y}{\eta^{\pi^{\asdep{\Gamma}}_x}_y}}{\StI{\pi^{\asdep{\Gamma}.x}_y;\pi^{\asdep{\Gamma}}_x}{\FI{(x, y)}}} : \F{z.\Sigma_1(\asdep{\Gamma}, \fst z, \snd z)}{\telety{x}{\upstairs{A}}{\upstairs{B}}}}
\end{mathpar}
...
\begin{enumerate}[style = multiline, labelwidth = 80pt]
\item[{$\mathsf{split}_{A,B}(\mathsf{pair}_{A,B}) \equiv \var{\Gamma, \Sigma_A B}$}:]
...
Now suppose $p$ supports strong $\Sigma$-types, so $\mathtt{pair}(\alpha,x,y)$ has an inverse
\begin{align*}
\mathtt{split}(\alpha,x,y) : \alpha.\Sigma_1(\alpha,x,y) \vDash\alpha.x.y
\end{align*}
This allows us to interpret strong elimination.
...
\begin{align*}
\inferrule*[Left=F-elim]{
\upstairs{\Gamma}, z : \upstairs{\Sigma_A B} \yields_z z : \upstairs{\Sigma_A B} \and 
\inferrule*[left=rewrite]{
\inferrule*[Left=cut]{
\inferrule*[Left=s-elim-2]{\upstairs{\Gamma}, x : \upstairs{A}, y : \upstairs{B} \yields_{1_{\asdep{\Gamma}.x.y}} \upstairs{c} : \St{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{C}[\FI{(x, y)}/z]}}
{\upstairs{\Gamma}, x : \upstairs{A}, y : \upstairs{B} \yields_{\TrCirc{\mathtt{pair}(\asdep{\Gamma},x,y)}{1_{\asdep{\Gamma}.x.y}}} \UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{c}} : \upstairs{C}[\FI{(x, y)}/z]}}
{\upstairs{\Gamma}, w : \telety{x}{\upstairs{A}}{\upstairs{B}} \yields_{\TrCirc{\mathtt{pair}(\asdep{\Gamma},\fst{w},\snd{w})}{1_{\asdep{\Gamma}.{\fst w}.{\snd w}}}} \UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{c}} : \upstairs{C}[\FI{w}/z]}}
{\upstairs{\Gamma}, w : \telety{x}{\upstairs{A}}{\upstairs{B}} \yields_{1_{\asdep{\Gamma}.w}[\Sigma_1(x,y)/w]} \rewrite{\ApCirc{\mathtt{pair}(\asdep{\Gamma},x,y)}{\varepsilon^{\mathtt{split}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.{\fst w}.{\snd w}}}}}{\UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{c}}} : \upstairs{C}[\FI{w}/z]}
}
{\upstairs{\Gamma}, z : \upstairs{\Sigma_A B} \yields_{1_{\asdep{\Gamma}.z}} \FE{z}{w}{\rewrite{\ApCirc{\mathtt{pair}(\asdep{\Gamma},x,y)}{\varepsilon^{\mathtt{split}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.{\fst w}.{\snd w}}}}}{\UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{c}}}} : \upstairs{C}} 
\end{align*}
...
Again, the Beck-Chevalley condition will appear when we check the equations:
\begin{enumerate}[style = multiline, labelwidth = 80pt]
