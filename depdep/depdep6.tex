...
\end{mathpar}
In the absence of eta for $\mathsf{F}$- and $s$-types, we also need to assert
\begin{mathpar}
...
\StI{s}{\UI{x}{M}} &\equiv \UI{x}{M} &\St{s}{\U{c.\mu}{x:A}{B}} &\equiv \U{c.\mu[\TrCirc{s}{c}/c]}{x:A}{B} \\
\UI{x}{\StI{s}{M}} &\equiv \UI{x}{M} &\U{c.\mu}{x:A}{\St{s}{B}} &\equiv \U{c.\TrCirc{s}{\mu}}{x:A}{B} \\
...
\end{align*}
using the other.
\mvrnote{This uses definitional eta for $s$-types?}
...
Our goal is to interpret these rules in the framework. The mode theory will contain a comprehension object $p$, possibly with some extra strictness \mvrnote{to be determined}. The basic idea for representing the judgements of MLTT is as follows:
\begin{itemize}
...
First all the structural rules.
...
Suppose $p$ supports $\Pi$-types. In the framework, $\Pi$-types are the $\mathsf{U}$-types for the mode term
\begin{align*}
...
&\equiv \UI{x}{\upstairs{b}[\upstairs{\Theta}, x / x]} && :  \U{-}{x : \upstairs{A}[\upstairs{\Theta}]}{\upstairs{B}[\upstairs{\Theta}, x / x]} \\
...
\begin{lemma}
Substitution commutes with $\mathtt{pair}$ at the mode theory level:
\begin{align*}
...
\begin{lemma}
The following difficult-to-motivate equation is true:
\begin{align*}
...
\end{lemma}
\begin{proof}
\mvrnote{Stuck? Is this something we have to assert?}
\begin{align*}
&\eta^{s_{\Theta}}_{\Sigma_1(\asdep{\Gamma},x,y)} \\
&\equiv ?? \\
&\equiv \ApCirc{\pi^{\Gamma_p}_x}{\eta^{(s_\Theta \bdot \id)}_y ; \ApPlus{(s_\Theta \bdot \id)}{\eta^{(\id \bdot \eta^{s_\Theta}_{x})}_{\TrCirc{(s_\Theta \bdot \id)}{y}}}} ; \ApCirc{\pi^{\asdep{\Gamma}}_{\TrPlus{s_\Theta}{\TrCirc{s_\Theta}{x}}}}{\varepsilon^{(\id \bdot \eta^{s_\Theta}_{x})}_{\TrPlus{(s_\Theta \bdot \id)}{\TrCirc{s_{\Theta \uparrow A}}{y}}}} \\
&\equiv \ApCirc{\pi^{\Gamma_p}_x}{\eta^{(s_\Theta \bdot \id);(\id \bdot \eta^{s_\Theta}_{x})}_y} ; \ApCirc{\pi^{\asdep{\Gamma}}_{\TrPlus{s_\Theta}{\TrCirc{s_\Theta}{x}}}}{\varepsilon^{(\id \bdot \eta^{s_\Theta}_{x})}_{\TrPlus{(s_\Theta \bdot \id)}{\TrCirc{s_{\Theta \uparrow A}}{y}}}} \\
&\equiv \ApCirc{\pi^{\Gamma_p}_x}{\eta^{s_{\Theta \uparrow A}}_y} ; \ApCirc{\pi^{\asdep{\Gamma}}_{\TrPlus{s_\Theta}{\TrCirc{s_\Theta}{x}}}}{\varepsilon^{(\id \bdot \eta^{s_\Theta}_{x})}_{\TrPlus{(s_\Theta \bdot \id)}{\TrCirc{s_{\Theta \uparrow A}}{y}}}} \\
&\equiv \ap{\Sigma_1(\asdep{\Gamma},\fst z, \snd z)}{\ap{(x, m)}{\eta^{s_{\Theta \uparrow A}}_y/m}/z} ;  \ap{\Sigma_1(\asdep{\Gamma},\fst z, \snd z)}{(\eta^{s_\Theta}_x, \id) /z} \\
&\equiv \ap{\Sigma_1(\asdep{\Gamma},\fst z, \snd z)}{\ap{(x, m)}{\eta^{s_{\Theta \uparrow A}}_y/m};(\eta^{s_\Theta}_x, \id)/z} \\
&\equiv \ap{\Sigma_1(\asdep{\Gamma},\fst z, \snd z)}{(\eta^{s_\Theta}_x, \eta^{s_{\Theta \uparrow A}}_y)/z}
\end{align*}
\end{proof}
...
\inferrule*[left=cut]{
\inferrule*[Left=s-elim-2]{
\inferrule*[Left=rewrite]{\upstairs{\Gamma}, x : \upstairs{A}, y : \upstairs{B} \yields_{1_{\asdep{\Gamma}.x.y}} \upstairs{c} : \St{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{C}[\FI{(x, y)}/z]}}
{\upstairs{\Gamma}, x : \upstairs{A}, y : \upstairs{B} \yields_{\TrCirc{\mathtt{split}(\asdep{\Gamma},x,y)}{1_{\asdep{\Gamma}.\Sigma_1(x,y)}}} \rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.x.y}}}{\upstairs{c}} : \St{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{C}[\FI{(x, y)}/z]}}} 
{\upstairs{\Gamma}, x : \upstairs{A}, y : \upstairs{B} \yields_{1_{\asdep{\Gamma}.\Sigma_1(x,y)}} \UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.x.y}}}{\upstairs{c}}} : \upstairs{C}[\FI{(x, y)}/z]}}
{\upstairs{\Gamma}, w : \telety{x}{\upstairs{A}}{\upstairs{B}} \yields_{1_{\asdep{\Gamma}.\Sigma_1(\fst w,\snd w)}} \UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\upstairs{c}[\fst w / x, \snd w / y]}} : \upstairs{C}[\FI{w}/z]}
}
{\upstairs{\Gamma}, z : \upstairs{\Sigma_A B} \yields_{1_{\asdep{\Gamma}.z}} \FE{z}{w}{\UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\upstairs{c}[\fst w / x, \snd w / y]}}} : \upstairs{C}} 
\end{align*}
\mvrnote{Uh oh, we use cut here. Is it in a way that breaks the extensibility of the translation? Note that the reason we're using cut is that the telescope types use $\pi_1$ and $\pi_2$ eliminators, which is a mismatch with the $\mathsf{split}$ style of everything else. Maybe this means we're ok?}
\end{enumerate}
...
&\equiv \StI{\mathtt{pair}(A,B)}{\upstairs{\mathsf{split}_{A,B}(c)}[\FI{(x, y)}/z]} \\
&\equiv \StI{\mathtt{pair}(A,B)}{\FE{z}{w}{\UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\upstairs{c}[\fst w / x, \snd w / y]}}}[\FI{(x, y)}/z]} \\
&\equiv \StI{\mathtt{pair}(A,B)}{\FE{\FI{(x, y)}}{w}{\UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\upstairs{c}[\fst w / x, \snd w / y]}}}} \\
&\equiv \StI{\mathtt{pair}(A,B)}{\UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\upstairs{c}[\fst w / x, \snd w / y]}}[(x,y)/w]} \\
&\equiv \StI{\mathtt{pair}(A,B)}{\UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.x.y}}}{\upstairs{c}}}} \\
&\equiv \rewrite{\ApPlus{\mathtt{pair}(\asdep{\Gamma},x,y)}{\ApCirc{\mathtt{pair}(\asdep{\Gamma},x,y)}{\varepsilon^{\mathtt{split}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.x.y}}}}}{\StI{\mathtt{pair}(A,B)}{\UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{c}}}} \\
&\equiv \rewrite{\eta^{\mathtt{pair}(\asdep{\Gamma},x,y)}_{1_{\asdep{\Gamma}.x.y}}}{\StI{\mathtt{pair}(A,B)}{\UnSt{\mathtt{pair}(\asdep{\Gamma},x,y)}{\upstairs{c}}}} \\
&\equiv \upstairs{c} \\
\end{align*}
...
\end{align*}
So we need to smuggle $\eta$ in to both sides. Let's try and see how to do that:
\begin{align*}
...
\end{align*}
So all that's left is to calculate the rewrite
\begin{align*}
...
&\equiv \StI{(s_\Theta \bdot \eta_z^{s_\Theta})}{\FE{z}{w}{\UnSt{\mathtt{pair}(\asdep{\Delta},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Delta},\fst w,\snd w)}_{1_{\asdep{\Delta}.\fst w . \snd w}}}{\upstairs{c}[\fst w / x, \snd w / y]}}}[\upstairs{\Theta}, \UnSt{s_\Theta}{z}/z]} \\
%&\equiv \StI{(s_\Theta \bdot \eta_z^{s_\Theta})}{\FE{\UnSt{s_\Theta}{z}}{w}{\UnSt{\mathtt{pair}(\asdep{\Delta}[\upstairs{\Theta}],\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Delta}[\upstairs{\Theta}],\fst w,\snd w)}_{1_{\asdep{\Delta}[\upstairs{\Theta}].\fst w . \snd w}}}{\upstairs{c}[\upstairs{\Theta}, \fst w / x, \snd w / y]}}}} \\
\end{align*}

\begin{align*}
&\equiv \FE{z}{w}{\UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\StI{s_{\Theta \uparrow A \uparrow B}}{\upstairs{c}[\upstairs{\Theta}, \UnSt{s_\Theta}{\fst w}/x,  \UnSt{s_{\Theta \uparrow A}}{\snd w}/y]}}}} \\
&\equiv \FE{z}{w}{\UnSt{\mathtt{pair}(\asdep{\Gamma},\fst w,\snd w)}{\rewrite{\varepsilon^{\mathtt{split}(\asdep{\Gamma},\fst w,\snd w)}_{1_{\asdep{\Gamma}.\fst w . \snd w}}}{\upstairs{c[\Theta \uparrow A \uparrow B]}[\fst w / x, \snd w / y]}}} \\
