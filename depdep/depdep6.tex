...
\newcommand\UStI[1]{\ensuremath{\mathsf{ust}(#1)}}
\newcommand\UnSt[1]{\ensuremath{\mathsf{unst}(#1)}}
...
\inferrule*[Left = s-intro-2]{
  ...
  {\Gamma \yields_{\mu} \UStI{M} : \St{s}{B}
  } \and
  
  \inferrule*[Left = s-elim-2]{
    \Gamma \yields_{\mu} N : \St{s}{B}
  }{
    \Gamma \yields_{\TrCirc{s}{\mu}} \UnSt{N} : B
  } 
\end{mathpar}
\end{lemma}
\begin{proof}
...
\inferrule*[Left = s-intro]{
  ...
  {\Gamma \yields_{\TrPlus{s}{\TrCirc{s}{\mu}}} \StI{M} : \St{s}{A}}}
  {
    {\Gamma \yields_{\mu} \UStI{M} :\equiv \rewrite{\eta^s_\mu}{\StI{M}} : \St{s}{A}}}
\end{mathpar}
And the second:
\begin{mathpar}
\inferrule*[Left = s-elim]{
...
\inferrule*[left = 2cell]{
  ...
  {
    \Gamma \yields_{\TrCirc{s}{\mu}} \UnSt{N} :\equiv \StE{N}{x}{\rewrite{\varepsilon^s_x}{x}} : B
  }
\end{mathpar}
\end{proof}
...
\begin{mathpar}
\qyields \Gamma \CTX \and \Gamma \qyields A \TYPE \and \Gamma \qyields a : A \and \Gamma \qyields \theta : \Delta 
\end{mathpar}
...
\inferrule*[left=type-sub]{\Delta \qyields A \TYPE \and \Gamma \qyields \theta : \Delta}{\Gamma \qyields A[\theta] \TYPE} \and
\inferrule*[left=term-sub]{\Delta \qyields a : A  \and \Gamma \qyields \theta : \Delta}{\Gamma \qyields a[\theta] : A[\theta]} 
\\
\inferrule*[left=sub-empty]{~}{\Gamma \qyields \epsilon : \cdot} \and
\inferrule*[left=sub-ext]{\Gamma \qyields \theta : \Delta \and \Gamma \qyields a : A[\theta]}{\Gamma \qyields (\theta, a) : \Delta, A} \\
\inferrule*[left=sub-id]{~}{\Gamma \qyields \id_\Gamma : \Gamma} \and
\inferrule*[left=sub-comp]{\Gamma \qyields \theta : \Delta \and \Delta \qyields \kappa : \Lambda}{\Gamma \qyields \theta ; \kappa : \Lambda} \\
\inferrule*[left=sub-$\pi_1$]{\Gamma \qyields \theta : \Delta, A}{\Gamma \qyields \pi_1(\theta) : \Delta} \and 
\inferrule*[left=sub-$\pi_2$]{\Gamma \qyields \theta : \Delta, A}{\Gamma \qyields \pi_2(\theta) : A[\pi_1(\theta)]} 
\end{mathpar}

\begin{align}
A[\id] &\equiv A \\
A[\theta ; \kappa] &\equiv A[\kappa][\theta] \\
\nonumber\\
a[\id] &\equiv a \\
a[\theta ; \kappa] &\equiv a[\kappa][\theta] \\
\nonumber\\
\id ; \theta &\equiv \theta \\
\theta ; \id &\equiv \theta \\
(\theta; \kappa) ; \rho &\equiv \theta ; (\kappa ; \rho) \\
\nonumber\\
\theta ; (\kappa , a) &\equiv (\theta ; \kappa) , a[\theta] \\ 
\pi_1(\theta, a) &\equiv \theta \\
\pi_2(\theta, a) &\equiv a \\
(\pi_1(\theta), \pi_2(\theta)) &\equiv \theta \\
\theta &\equiv \epsilon && \text{for } \Gamma \qyields \theta : \cdot
\end{align}
...
\begin{mathpar}
\inferrule*[left=derivable]{\Gamma \qyields \theta : \Delta \and \Delta \qyields A \TYPE}{\Gamma, A[\theta] \qyields \theta \uparrow A : \Delta, A} \and
\inferrule*[left=derivable]{\Gamma \qyields a : A}{\Gamma \qyields \overline{a} : \Gamma, A}
\end{mathpar}
defined by:
\begin{align*}
\theta \uparrow A &:\equiv (\pi_1(\id_{\Gamma, A[\theta]}); \theta) , \pi_2(\id_{\Gamma, A[\theta]}) \\
\overline{a} &:\equiv \id_{\Gamma} , a
\end{align*}
...
\end{mathpar}
\begin{align}
(\Pi_A B)[\theta] &\equiv \Pi_{A[\theta]} B[\theta \uparrow A] \\
\nonumber\\
\mathsf{app}(\mathsf{lam}(b)) &\equiv b \\
\mathsf{lam}(\mathsf{app}(f)) &\equiv f \\
\mathsf{lam}(b)[\theta] &\equiv \mathsf{lam}(b[\theta \uparrow A])
\end{align}
...
\inferrule*[left=$\Sigma$-pair]{\Gamma \qyields a : A \and \Gamma \qyields b : B[\overline{a}]}{\Gamma \qyields (a, b) : \Sigma_A B} \and
...
\end{mathpar}
\begin{align}
(\Sigma_A B)[\theta] &\equiv \Sigma_{A[\theta]} B[\theta \uparrow A] \\
\nonumber\\
\pi_1(a, b) &\equiv a \\
\pi_2(a, b) &\equiv b \\
(\pi_1(p), \pi_2(p)) &\equiv p \\
(a, b)[\theta] &\equiv (a[\theta], b[\theta])
\end{align}
\caption{Rules for $\Pi$-types in MLTT via Explicit Substitutions}\label{fig:qit-sigma-rules}
\end{figure}

Our goal is to interpret these rules in the framework. The mode theory consists of an object with finite limits $p$ that supports both stable $\Sigma$s and $\Pi$s. The basic idea for representing the judgements of MLTT is as follows:
\begin{itemize}
...
\inferrule*[Left=Sub]{
  ...
  \downstairs{\Gamma} \yields s_\Theta : \asdep{\Gamma} \vDash \asdep{\Delta}[\downstairs{\Theta}]
    ...
    \upstairs{\Gamma} \yields_{\asdep{\Gamma}} s_{\Theta}^*(\upstairs{A}[\upstairs{\Theta}]) \TYPE
  }
\end{mathpar}
...
\inferrule*[Left=Sub]{
 \upstairs{\Delta} \yields_{1_{\asdep{\Delta}}} \upstairs{a} : \upstairs{A} \and 
  ...
  \downstairs{\Gamma} \yields s_\Theta : \asdep{\Gamma} \vDash \asdep{\Delta}[\downstairs{\Theta}]
    ...
    \upstairs{\Gamma} \yields_{\TrPlus{s_{\Theta}}{1_{\asdep{\Delta}[\downstairs{\Theta}]}}} \StI{\upstairs{a}[\upstairs{\Theta}]} : \St{s_{\Theta}}{\upstairs{A}[\upstairs{\Theta}]}
  }
\end{mathpar}
...
\end{align*}
From the term, we get another term $\upstairs{\Gamma}\yields_{\TrCirc{s_{\Theta}}{1_{\asdep{\Gamma}}}} \UnSt{\upstairs{a}} : \upstairs{A}[\upstairs{\Theta}]$, where I use $\UnSt{\upstairs{a}}$ as shorthand for $\StE{\upstairs{a}}{x}{x}$. \mvrnote{There is also a hidden use of the counit for $s_{\Theta}$ here} We can now pair this with $\upstairs{\Theta}$ to get a framework substitution \[\upstairs{\Gamma} \yields_{\downstairs{\Theta}, \TrCirc{s_{\Theta}}{1_{\asdep{\Gamma}}}} (\upstairs{\Theta}, \UnSt{\upstairs{a}}) : \upstairs{\Delta}, \upstairs{A}\]
so we define
\begin{align*}
\upstairs{\Theta, a/x} &:\equiv \upstairs{\Theta}, \UnSt{\upstairs{a}}/x \\
...
&\equiv \asdep{\Delta}.x[\downstairs{\Theta}][\TrCirc{s_{\Theta}}{1_{\asdep{\Gamma}}}/x] \\ 
&\equiv \asdep{\Delta}[\downstairs{\Theta}].\TrCirc{s_{\Theta}}{1_{\asdep{\Gamma}}}
\end{align*}
So define 
\begin{align*}
s_{\Theta, a} &\equiv \eta^\chi_{\downstairs{\Gamma}}; (s_\Theta \bdot \eta_{1_{\asdep{\Gamma}}}^{s_\Theta})
\end{align*}
...
\item[\textsc{sub-$\pi_1$}] This is translated to the framework substitution with the last term forgotten. We are given
\begin{align*}
\downstairs{\Gamma} \yields s_\Theta : \asdep{\Gamma} \vDash_p \asdep{(\Delta, x : A)}[\downstairs{\Theta}]
\end{align*}
and need to produce
\begin{align*}
\downstairs{\Gamma} \yields s_{\pi_1(\Theta)} : \asdep{\Gamma} \vDash_p \asdep{\Delta}[\pi_1(\downstairs{\Theta})]
\end{align*}
For this we can use $s_{\pi_1(\Theta)} :\equiv s_\Theta;\pi^{\asdep{\Delta}[\downstairs{\Theta}]}_{x[\downstairs{\Theta}]}$

\item[\textsc{sub-$\pi_2$}] We have a framework substitution 
\begin{align*}
\upstairs{\Gamma} &\yields \upstairs{\Theta} : \upstairs{\Delta, x : A} \\
\downstairs{\Gamma} &\yields s_\Theta : \asdep{\Gamma} \vDash_p \asdep{(\Delta, x : A)}[\downstairs{\Theta}]
\end{align*}
and WLOG $\upstairs{\Theta} \equiv (\upstairs{\Theta'}, a)$ for some $\upstairs{\Gamma} \yields_\mu a : \upstairs{A}[\upstairs{\Theta'}]$, where $\downstairs{\Gamma} \yields \mu : \asdep{\Delta}[\downstairs{\Theta'}]$. Notice also that
\begin{align*}
\asdep{(\Delta, x : A)}[\downstairs{\Theta}] 
&\equiv \asdep{\Delta}.x[\downstairs{\Theta'}, \mu/x] \\
&\equiv \asdep{\Delta}[\downstairs{\Theta'}].\mu
\end{align*}

Our goal is to produce a framework term 
\begin{align*}
\upstairs{\Gamma} \yields_{1_{\asdep{\Gamma}}} \pi_2(\Theta) : s_{\pi_1(\Theta)}^*(\upstairs{A}[\upstairs{\pi_1(\Theta)}])
\end{align*}
The type there is equal to:
\begin{align*}
s_{\pi_1(\Theta)}^*(\upstairs{A}[\upstairs{\pi_1(\Theta)}])
&\equiv {(s_{\Theta};\pi)}^*(\upstairs{A}[\upstairs{\Theta'}]) \\
&\equiv s_{\Theta}^*(\pi^*(\upstairs{A}[\upstairs{\Theta'}]))
\end{align*}
Where $\pi :\equiv \pi^{\asdep{\Delta}[\downstairs{\Theta'}]}_{\mu}$ to reduce clutter.
Now use the derivation:
\begin{mathpar}
\inferrule*[Left=s-intro]
{
\inferrule*[Left=2cell]
...
{
 \inferrule*[Left=2cell] {
 \upstairs{\Gamma} \yields_\mu a : \upstairs{A}[\upstairs{\Theta'}] \and E(\mu) : \TrCirc{\ApEl{p}{\pi}}{1_{\asdep{\Delta}[\downstairs{\Theta'}].\mu}} \vDash_{\El{p}{\asdep{\Delta}[\downstairs{\Theta'}]}} \mu
 }{
 \upstairs{\Gamma} \yields_{\TrCirc{\pi}{1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]}}} E(x)(a) : \upstairs{A}[\upstairs{\Theta'}]
}}
{
\upstairs{\Gamma} \yields_{\TrPlus{\pi}{\TrCirc{\pi}{1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]}}}} \StI{E(x)(a)} : \St{\pi}{\upstairs{A}[\upstairs{\Theta'}]}
} \and \eta^{\pi}_{1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]}} : 1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]} \vDash \TrPlus{\pi}{\TrCirc{\pi}{1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]}}}}
{
\upstairs{\Gamma} \yields_{1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]}} \eta^{\pi}_1(\StI{E(x)(a)}) : \St{\pi}{\upstairs{A}[\upstairs{\Theta'}]}
}}
{
\upstairs{\Gamma} \yields_{1_{\asdep{\Gamma}}} \StI{\eta^{\pi}_1(\StI{E(x)(a)})} : \St{s_\Theta}{\St{\pi}{\upstairs{A}[\upstairs{\Theta'}]}}
}
\end{mathpar}
Where $E(x) :\equiv \ap{\ApEl{p}{\pi^\alpha_x}^\circ}{\var{x}} ; \varepsilon^{\ApEl{p}{\pi^\alpha_x}}_x$ is the counit of the $p/\alpha \to \El{p}{\alpha}$ adjunction. In the last step we again use Equation~\eqref{s-plus-one-strict} to identify $\TrPlus{s_\Theta}{1_{\asdep{(\Delta,x:A)}[\downstairs{\Theta}]}} \equiv 1_{\asdep{\Gamma}}$. 
