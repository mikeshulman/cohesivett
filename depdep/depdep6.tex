...
\ap q {s/\_} \equiv \id_q \\ 
\ap {(q[\mu/x])} {s/y} \equiv \ap q {\ap \mu {s/y}/x} \quad (\text{where } \gamma,x:p' \vdash \mu : p \text{ and } \gamma,x:p \vdash q \type)\\
...
\begin{mathpar}
  ...
  \ap{(\sigmacl{x'}{p}{q})}{s/y} \equiv \sigmacl{x'}{\ap{p}{s/y}}{? \ap{q}{s/y} ?}
\end{mathpar}
\drlnote{need to use encoded ap for second-last variable in this
  equation: add use of $\Sigma$-mode to do it}
...
\inferrule*{\TermTwoT{\gamma,x:p}{s}{\nu}{\nu}{q} \\
            \TermTwoT{\gamma}{t}{\mu}{\mu'}{p}\\
           } 
           {\TermTwoT{\gamma}{\ap s {t/x}}{\nu[\mu/x]}{\TrPlus{\ap{q}{s/x}}{\nu'[\mu'/x]}}{q[\mu/x]}}
...
\ap \mu {s/\_} \equiv \id_q \and
\ap {(\nu[\mu/x])} {s/y} \equiv \ap \nu {\ap \mu {s/y}/x} \quad
(\text{where } \gamma,x:p' \vdash \mu : p \text{ and } \gamma,x:p \vdash \nu : q \type)\\
t[\mu/x];\ap{\nu'}{s/x} \equiv \ap{\nu}{s/x};\ap{(\TrPlus{\ap{q}{s/x}}{y})}{t[\nu'/x]/y} \quad
...
Finally, we have the 2-cells for $\Sigma$-mode term constructors:
...
\St{\id_p}{A} &\equiv A\\
\St{(\ap{s}{t/x})}{A[M/x]} & \equiv (\St{s}{A})[\St{t}{M}/x]
\end{align*}
