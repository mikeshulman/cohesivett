...
\begin{mathpar}
  ...
  \inferrule*[Left = F-Elim]{
    ...
    \and (\text{where } \gamma, x:p \yields r [\mu / x] \type )}
...
over them:
\begin{mathpar}
p \type \and x : p \vdash S(x) \type \\
q \type \and y : q \vdash T(y) \type 
\end{mathpar}
A \emph{morphism of bifibrations} from $(p,S)$ to $(q,T)$ consists of terms
\begin{mathpar}
x : p \vdash f_0(x) : q \and 
x : p, z:S \vdash f_1(x,z) : T[f_0(x)/y]
\end{mathpar}
...
\begin{definition}[Strict Comprehension Object]
  ...
  \begin{mathpar}
    ...
    \TermTwoT{\alpha : p, x : \El{p}{\alpha}}{\,\bang_x}{x}{1_\alpha}{\El{p}{\alpha}} \\
    \alpha : p, x : \El{p}{\alpha} \yields \alpha.x : p \\
  \end{mathpar}
...
%% \ApEl{p}{s}^+(\ApEl{p}{\pi^\alpha_\mu}^\circ(\nu)) &\equiv
%% \ApEl{p}{\pi^\beta_{\TrPlus{\ApEl{p}{s}}{\mu}}}^\circ(\ApEl{p}{\tilde{s}}^+(\nu)) && \text{where } \gamma \vdash \nu : \El{p}{\alpha . \mu}\\
...
is a morphism of bifibrations from
$(\sigmacl{\alpha}{p}{\El{p}{\alpha}}, z.\El{p}{\fst z \bdot \snd z})$
to  
$(p,x.\El{p}{x})$
\end{center}

\noindent In the equations, we use the following derived forms:
\begin{itemize}
\item Pairing for the comprehension object is given by ap of
  ...
  By \eqref{dot-one-strict}, this has the usual comprehsion for ``substitutions''
  (mode term morphisms in $p$) as a special case, if we take the ``terms'' to be maps
  \TermTwoT{\gamma}{m}{1}{\ApEl{p}{s}^+(\nu)}{\El{p}{\alpha}}
  in the fiber:
  \begin{mathpar}
  \inferrule{\TermTwoT{\gamma}{s}{\alpha}{\beta}{p} \and
             %% \and \gamma \yields \mu : \alpha \and \gamma \yields \nu : \beta \and
             \TermTwoT{\gamma}{m}{1}{\ApEl{p}{s}^+(\nu)}{\El{p}{\alpha}}}
            {\TermTwoT{\gamma}{s \bdot m}{\alpha}{\beta.\nu}{p}}
  \end{mathpar}

  As another special case, we have
\begin{mathpar}
    \inferrule{\TermTwoT{\gamma}{s}{\alpha}{\beta}{p} \and \gamma \yields \mu : \beta}
    {\TermTwoT{\gamma}{\tilde{s}}{\alpha.\ApEl{p}{s}^+(\mu)}{\beta.\mu}{p}}
    \and
    \tilde{s} :\equiv (s \bdot \id_{\ApEl{p}{s}^+(\mu)}) 
\end{mathpar}
  
\item Also using \eqref{dot-one-strict}, ``first projection'' from a ``context'' is given by 
\begin{mathpar}
  {\TermTwoT{\alpha:p,x:\El{p}{\alpha}}{\pi^\alpha_x}{\alpha.x}{\alpha.1_x \equiv \alpha}{p}}
  \and
  \pi^\alpha_x :\equiv (\id_\alpha \bdot \bang_x) \equiv \ap{\alpha.x}{\bang/(x:\El{p}{\alpha})} \\
\end{mathpar}
...
\item Projection of the last ``variable'' from a ``context'' is defined
  ...
  \ApEl{p}{\pi}^+$:
\begin{mathpar}
  {\TermTwoT{\alpha:p,x:\El{p}{\alpha}}{\mathsf{var}_x}{1_{\alpha.x}}{\ApEl{p}{\pi}^+(x)}{\El{p}{\alpha.x}}} 
    \and
    \mathsf{var}_x :\equiv \eta^{\ApEl{p}{\pi^\alpha_x}}(1_{\alpha.x}) 
  \end{mathpar}
...
\paragraph{Upstairs Types}

The mode terms for upstairs $\Sigma$ and $\Pi$ types are
\begin{align*}
\alpha : p, x : \El{p}{\alpha}, y : \El{p}{\alpha.x} \yields \Sigma_1(x,y) :\equiv \TrCirc{\ApEl{p}{\pi^\alpha_x}}{y}  : \El{p}{\alpha}\\
\alpha : p, x : \El{p}{\alpha}, y : \El{p}{\alpha} \yields \Pi_1(x,y) :\equiv \TrPlus{\ApEl{p}{\pi^\alpha_x}}{y} : \El{p}{\alpha.x}
\end{align*}
...
Now we match spacial type theory judgements with the mode theory judgements they live over:
\begin{align*}
&\yields \Delta;\Gamma & \gamma &\yields \underline{(\Delta;\Gamma)} : p \\
\Delta ;\Gamma &\yields A \TYPE & \gamma &\yields \El{p}{\underline{\Delta;\Gamma}} \type \\
\Delta;\Gamma &\yields M : A & \gamma &\yields 1_{(\underline{\Delta;\Gamma})} : \El{p}{\underline{\Delta;\Gamma}}
\end{align*}
Where
\begin{align*}
\underline{(\cdot ; \cdot)} &:\equiv \emptyset \\
\underline{(\Delta, x :: A ; \cdot)} &:\equiv \underline{(\Delta ; \cdot)}.\flat_1(x) \\
\underline{(\Delta ; \Gamma, y : B)} &:\equiv \underline{(\Delta ; \Gamma )}.y \\
\end{align*}
So that
\begin{align*}
f_0(\underline{\Delta ; \Gamma}) \equiv (\underline{\Delta, \Gamma ; \cdot})
\end{align*}
by idempotence.

\setlength{\jot}{20pt}
\begin{align*}
\inferrule*[vcenter, Left = var]{~}
{\Delta ; \Gamma, x : A \yields x : A} 
&&
\inferrule*[Right = 2cell]{
\inferrule*[Right = S-intro]{
\inferrule*[Right = var]{~}
  {\Gamma, x : A \yields_{x} x : A}}
  {\Gamma, x : A \yields_{\pi^+(x)} x : \pi^*(A)} \and ~ \and
    \TermTwo{\Gamma, x : A }{\mathsf{var}_x}{1_{\underline{(\Delta ; \Gamma, x:A )}}}{\pi^+(x)}}
  {\Gamma, x : A \yields_{1_{\underline{(\Delta ; \Gamma, x:A )}}} x : \pi^*(A)} \\
%
\inferrule*[vcenter, Left = crisp-var]{~}
...
\inferrule*[Right = var]{~}
  {\Gamma, x : A \yields_{x} x : A}}
  {\Gamma, x : A \yields_{\pi^+(x)} x : \pi^*(A)} \and ~ \and
    \TermTwo{\Gamma, x : A }{\mathsf{var}_x}{1_{\underline{(\Delta, \cdot )}.x}}{\pi^+(x)}}
  {\Gamma, x : A \yields_{1_{\underline{(\Delta, \cdot )}.x}} x : \pi^*(A)}}
  {\Gamma, x : A \yields_{1_{\underline{(\Delta, \cdot )}.\flat_1(x)}} x : (c_{f_0(\alpha).x};\pi)^*(A)}
  \\
\inferrule*[vcenter, Left = subst]{\Gamma \yields M : A \and \Delta ; \Gamma, x : A \yields N : B}
{\Delta ; \Gamma \yields N[M/x] : B[M/x]} 
&&\inferrule*[vcenter, Right= subst]{\Gamma \yields_{1_{\underline{(\Delta, \Gamma )}}} M : A \and \Gamma, x : A \yields_{1_{\underline{(\Delta, \Gamma)}.x}} N : B}
{\Gamma \yields_{1_{\underline{(\Delta, \Gamma)}.1_{\underline{(\Delta, \Gamma )}}}} N[M/x] : B[M/x]} 
  \\
\inferrule*[vcenter, Left = crisp-subst]{\Delta ; \cdot \yields M : A \and \Delta, x :: A ; \Gamma \yields N : B}
{\Delta ; \Gamma \yields N[M/x] : B[M/x]} 
&&\inferrule*[vcenter, Right= subst]{\Gamma \yields_{1_{\underline{(\Delta, \cdot )}}} M : A \and \Gamma, x : A \yields_{1_{\underline{\Delta}.\flat_1(x).\pi^+\underline{\Gamma}}} N : B}
{\Gamma \yields_{1_{\underline{\Delta}.\underline{\Gamma}}} N[M/x] : B[M/x]} 
\end{align*}
...
\begin{align*}
\inferrule*[vcenter, Left = $\flat$-form]{\Delta ; \cdot \yields A \TYPE}
{\Delta ; \cdot \yields \flat A \TYPE} 
&&\inferrule*[vcenter, Right = F-form]{
    \Gamma \yields_{\El{p}{f_0(\alpha)}} A \TYPE\\\\
    \gamma, x : {\El{p}{f_0(\alpha)}} \yields \flat_1(\alpha,x) : \El{p}{f_0(\alpha)}
  }
  {\Gamma \yields_{\El{p}{f_0(\alpha)}} \F{x.\flat_1(\alpha,x)}{A} \TYPE} \\
%
\inferrule*[vcenter, Left = $\flat$-intro]{\Delta ; \cdot \yields M : A}
{\Delta ; \cdot \yields M^\flat : \flat A} &&
\inferrule*[vcenter, Right = F-intro]{
    \Gamma \yields_{1_{f_0(\alpha)}} M : A
  }
  {\Gamma \yields_{\flat_1(\alpha,x)[1/x] \equiv 1_{f_0(\alpha)}} \FI{M} : \F{x.\flat_1(\alpha,x)}{A}} \\
%
\inferrule*[vcenter, Left=crisp-$\flat$-elim]{
    \Delta, x :: \flat A ; \cdot \yields C \TYPE \\\\
    \Delta ; \cdot \yields M : \flat A \\\\
    \Delta, u :: A ; \cdot \yields N : C[u^\flat / x]}
  {\Delta ; \cdot \yields (\flatE{M}{u}{N}) : C[M / x]}
 &&  \inferrule*[vcenter, Right = F-elim]{
    \Gamma, y : \F{x.\flat_1(\alpha,x)}{A} \yields_{\underline{(\Delta ; \cdot )}.\flat_1(y)} C \TYPE \\\\
    \Gamma \yields_{1_{\underline{(\Delta ; \cdot )}}} M : \F{x.\flat_1(\alpha,x)}{A} \\\\
    \Gamma, x:A \yields_{1_{\underline{(\Delta ; \cdot)}.\flat_1(y)}[\flat_1(\alpha,x) / y]} N : C [\FI{x}/y]}
  {\Gamma \yields_{1_{\underline{(\Delta ; \cdot )}.\flat_1(y)}[1_{\underline{(\Delta ; \cdot )}}/y]} \FE{M}{x}{N} : C[M/y]} \\
%
\inferrule*[vcenter, Left=$\flat$-elim]{
 ...
 &&  \inferrule*[vcenter, Right = F-elim]{
    \Gamma, y : \F{x.\pi_\Gamma^+(\flat_1(\alpha,x))}{A} \yields_{\underline{(\Delta ; \Gamma )}.y} C \TYPE \\\\
    \Gamma \yields_{1_{\underline{(\Delta ; \Gamma )}}} M : \F{x.\pi_\Gamma^+(\flat_1(\alpha,x))}{A} \\\\
    \Gamma, x:A \yields_{1_{\underline{(\Delta ; \Gamma )}.y}[\pi_\Gamma^+(\flat_1(\alpha,x)) / y]} N : C [\FI{x}/y]}
  {\Gamma \yields_{1_{\underline{(\Delta ; \Gamma )}.y}[1_{\underline{(\Delta ; \Gamma )}}/y]} \FE{M}{x}{N} : C[M/y]}
\end{align*}
In crisp-$\flat$-elim, we must use idempotence to show $1_{\underline{(\Delta ; \cdot)}.\flat_1(y)}[\flat_1(\alpha,x) / y] \equiv 1_{\underline{(\Delta ; \cdot)}.\flat_1(\alpha,x)}$

In $\flat$-elim, to get from $1_{\underline{(\Delta ; \Gamma )}.y}[\pi_\Gamma^+(\flat_1(\alpha,x)) / y]$ back to $1_{\underline{(\Delta, y : A ; \Gamma)}}$, we have to push the $y$ past everything in $\Gamma$, then push the new $\pi^+$s on the entries of $\underline{\Gamma}$ into the types in $\Gamma$. (or something)
\begin{align*}
\inferrule*[vcenter, Left = $\sharp$-form]{\Delta, \Gamma ; \cdot \yields A \TYPE}
{\Delta ; \Gamma \yields \sharp A \TYPE} 
&&  \inferrule*[vcenter, Right = U-form]{
    \Gamma \yields_{f_0(\underline{\Delta ; \Gamma})} A \TYPE \\\\
    \and \gamma, c : {\El{p}{\underline{\Delta ; \Gamma}}} \yields f_1(\underline{\Delta ; \Gamma},c) : \El{p}{f_0(\underline{\Delta ; \Gamma})}
  }{\Gamma \yields_{\underline{(\Delta ; \Gamma)}} \U{c.f_1(\underline{\Delta ; \Gamma},c)}{\cdot}{A} \TYPE} \\
%
\inferrule*[vcenter, Left = $\sharp$-intro]{\Delta, \Gamma ; \cdot \yields M : A}
{\Delta ; \Gamma \yields M^\sharp : \sharp A} 
&&\inferrule*[vcenter, Right = U-intro]{
    \Gamma \yields_{f_1(\alpha,c)[1_{(\underline{\Delta ; \Gamma})}/c]\equiv 1_{f_0(\underline{\Delta ; \Gamma})}}  M : A
  }
  {\Gamma \yields_{1_{(\underline{\Delta ; \Gamma})}} \lambda{M} : \U{c.f_1(\alpha,c)}{\cdot}{A}
  } \\
%
\inferrule*[vcenter, Left = $\sharp$-elim]{\Delta ; \Gamma \yields M : \sharp A}
{\Delta, \Gamma ; \cdot \yields M_\sharp : A} 
&&  \inferrule*[vcenter, Left = U-elim]{
    \Gamma \yields_{1_{(\underline{\Delta ; \Gamma})}} N : \U{c.f_1(\alpha,c)}{\cdot}{A}
  }{
    \Gamma \yields_{f_1(\alpha,c)[1_{(\underline{\Delta ; \Gamma})}/c]} \UE{N}{\cdot} : A
  } \\
\end{align*}
