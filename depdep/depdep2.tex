...
\begin{mathpar}
\inferrule{~}
          {\gamma \vdash \emptyset \type} \and
\inferrule{\gamma \vdash \sigma : q \Rightarrow p \type \and \gamma \vdash \mu : q}
          {\gamma \vdash p.\mu(\sigma) \type} \and
\end{mathpar}
...
\begin{mathpar}
\inferrule{~}
          {\gamma \vdash !_p : \emptyset \Rightarrow p \type} \and
\inferrule{\gamma \vdash \tau : p \Rightarrow r \type \and \gamma \vdash s : \mu \Rightarrow (\sigma ; \tau)_*(\just{r}) }
          {\gamma \vdash (\tau, s) : p.\mu(\sigma) \Rightarrow r \type} \\
\inferrule{~}
          {\gamma \vdash \mathsf{inl} : p \Rightarrow p.\mu(s) \type} \and
\inferrule{~}
          {\gamma \vdash \mathsf{inr} : \mu \Rightarrow (\sigma ; \mathsf{inl})_*(\just{p.\mu(\sigma)}) \type}
\end{mathpar}
...
\inferrule{~}
          {\gamma \vdash () \type} \and
\inferrule{(x : n) \in \gamma \and \sigma : n \leadsto m \subst}
          {\gamma \vdash (m, a : x[\sigma]) \type} \and
\\
\inferrule{~}
          {\gamma \vdash [] : () \leadsto m \subst} \and
\inferrule{\gamma \vdash \tau : n \leadsto m \subst \and \gamma \vdash (b : x[\tau ; \sigma]) \in m}
          {\gamma \vdash [\tau, a \mapsto b] : (n, a : x[\sigma]) \leadsto m \subst} \and
\\
\inferrule{~}
          {\gamma \vdash (a : x[\sigma]) \in (m, a : x[\sigma])} \and 
\inferrule{\gamma \vdash (a : x[\sigma]) \in m}
          {\gamma \vdash (a : x[\sigma]) \in (m, b : y[\tau])}
\end{mathpar}
Where the composition $[\tau ; \sigma] \subst$ is admissible. 

Taking an example from that paper, there is a FIC with three objects
\begin{tikzcd}
O & A \ar[l, bend left] \ar[l, bend right] & I \ar[l]
\end{tikzcd}
...
\begin{mathpar}
\inferrule{\gamma \vdash n \type \and \gamma \vdash \sigma : m \leadsto n \subst}
          {\gamma \vdash \mathsf{via}(\sigma) : m} \and
\end{mathpar}
