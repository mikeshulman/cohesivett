...
When the context is implicit, we write $\TermTwoT{\gamma}{s}{\mu}{\mu'}{p}$ as $s : \mu \tcell_p \mu'$. 
...
\begin{mathpar}
\inferrule{\gamma \yields t : \theta \tcell \theta' \and \gamma \yields s : \mu[\theta] \tcell \TrPlus{\ap{p}{t}}{\mu'[\theta']}}
{ \gamma \yields (s, t) : (\theta, \mu) \tcell (\theta', \mu') : (\delta,x:p)} \\
%
\inferrule{\gamma \yields t : ((\theta, \mu) \tcell (\theta', \mu')) : (\delta, x : p)}
{\gamma \yields \ap{\fst}{t} : \theta \tcell \theta' } \and
%
\inferrule{\gamma \yields t : ((\theta, \mu) \tcell (\theta', \mu')) : (\delta, x : p)}
{\gamma \yields \ap{\snd}{t} : (\mu[\theta] \tcell \TrPlus{\ap{p}{\ap{\fst}{t}}}{\mu'[\theta']})}
\end{mathpar}%

\begin{lemma}
N-ary ap of a 2-cell on a substitution is admissible
\begin{mathpar}
\inferrule{\gamma \yields t : \theta \tcell \theta' \and \delta \yields \kappa : \lambda}
{\gamma \yields \ap{\kappa}{t} : \kappa[\theta] \tcell \kappa[\theta'] : \lambda}
\end{mathpar} 
\end{lemma}
\begin{proof}
...
Let $f$ be a morphism of comprehension objects from $p$ to itself. We give $\TrPlus{f}{-}$ the structure of a comonad by adding morphisms
\begin{align*}
...
Define:
\begin{align*}
\alpha : p, x : \El{p}{\TrPlus{f}{\alpha}} \yields \flat_1(x) :\equiv \TrPlus{\ApEl{p}{\fcomult{\alpha}}}{f_1(x)} : \El{p}{\TrPlus{f}{\alpha}}
\end{align*}
...
\newcommand{\FlatE}[1]{\mathsf{open}({#1})}

The rules for spatial types are given in Figure~\ref{fig:qit-spatial-rules}.

The last component of the substitution into $C$ needs to be a term of type $\Flattype{(A[\counit{\Gamma}])}[\proj{\Gamma, A}, \lock]$

Start with $\FlatI{A[\counit{\Gamma}]}$ in context $\Gamma, \lock, A[\counit{\Gamma}]$, this has type:
\begin{align*}
\Flattype{(A[\counit{\Gamma}])}[\proj{\Gamma, \lock, A[\counit{\Gamma}]}]
\end{align*}
We want a substitution $\Gamma, A, \lock \qyields \Gamma, \lock, A[\counit{\Gamma}]$. We can get this by
\begin{align*}
\proj{\Gamma, A}, \lock, \qvar{A}[\counit{\Gamma, A}]
\end{align*}
So then
$\FlatI{A[\counit{\Gamma}]}[\proj{\Gamma, A}, \lock, \qvar{A}[\counit{\Gamma, A}]]$ indeed has type
\begin{align*}
&\Flattype{(A[\counit{\Gamma}])}[\proj{\Gamma, \lock, A[\counit{\Gamma}]}][\proj{\Gamma, A}, \lock, \qvar{A}[\counit{\Gamma, A}]] \\
&\equiv \Flattype{(A[\counit{\Gamma}])}[\proj{\Gamma, A}, \lock, \qvar{A}[\counit{\Gamma, A}];\proj{\Gamma, \lock, A[\counit{\Gamma}]}] \\
&\equiv \Flattype{(A[\counit{\Gamma}])}[\proj{\Gamma, A}, \lock]
\end{align*}
...
%\inferrule*[left=R-intro]{\Gamma, \lock \qyields_q a : A}{\Gamma \qyields_p \RI{a} : \Rtype{A}} \and
%\inferrule*[left=R-elim]{\Gamma \qyields_p b : \Rtype{B}}{\Gamma, \lock \qyields_q \RE{b} : B} \\
\inferrule*[left=counit-$\lock$]{~}{\Gamma, \lock \qyields \counit{\Gamma} : \Gamma} \\
\inferrule*[left=$\flat$-form]{\Gamma, \lock \qyields A \TYPE}{\Gamma, \lock \qyields \Flattype{A} \TYPE} \\
\inferrule*[left=$\flat$-intro]{~}{\Gamma, \lock, A \qyields \FlatI{A} : \Flattype{A}[\proj{\Gamma, \lock, A}]} \and
\inferrule*[left=$\flat$-elim]{\Gamma, A, \lock \qyields c : C[\proj{\Gamma, A}, \lock, \FlatI{A[\counit{\Gamma}]}[\proj{\Gamma, A}, \lock, \qvar{A}[\counit{\Gamma, A}]]]}{\Gamma, \lock, \Flattype{(A[\counit{\Gamma}])} \qyields \FlatE{c} : C} \\
\end{mathpar}
\begin{align}
%\id_\Gamma , \lock &\equiv \id_{\Gamma, \lock} \\
%(\Theta, \lock) ; (\kappa, \lock) &\equiv (\Theta ; \kappa), \lock \\
...
%\LE{c}[\proj{\Gamma, A}, \lock, \LI{A}] &\equiv c
\end{align}
\caption{Rules for Spatial Type Theory with Explicit Substitutions}\label{fig:qit-spatial-rules}
\end{figure}
