...
\begin{itemize}
\item A context $\qyields \Gamma \CTX$ is represented by a framework context $\upstairs{\Gamma}$ over $\downstairs{\Gamma}$ and a mode term $\modeof{\Gamma} : p$.
...
\item A substitution $\Gamma \qyields \Theta : \Delta$ is represented by a term $\upstairs{\Gamma} \qyields \upstairs{\Theta} : \mathsf{packed}(\upstairs{\Delta})$ where \mvrnote{we can probably define packed in terms of stuff we already have}
\end{itemize}
