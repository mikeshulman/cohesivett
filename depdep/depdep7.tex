...
\inferrule*[left=$\flat$-form]{\Gamma, \lock \qyields A \TYPE}{\Gamma, \lock \qyields \Flattype{A} \TYPE} \and
\inferrule*[left=$\flat$-intro]{~}{\Gamma, \lock, A, \lock \qyields \FlatI{A} : \Flattype{A}[\counit{\Gamma, \lock, A};\proj{\Gamma, \lock, A}]} \and
\inferrule*[left=$\flat$-elim]{\Gamma, \lock, A, \lock \qyields c : C[(\counit{\Gamma, \lock, A};\proj{\Gamma, \lock, A}), \FlatI{A}]}{\Gamma, \lock, \Flattype{A} \qyields \FlatE{c} : C} \\
\inferrule*[left=$\sharp$-form]{\Gamma, \lock \qyields A \TYPE}{\Gamma \qyields \Sharptype{A} \TYPE} \\
\inferrule*[left=$\sharp$-intro]{\Gamma, \lock \qyields a : A}{\Gamma \qyields \SharpI{a} : \Sharptype{A}} \and
\inferrule*[left=$\sharp$-elim]{\Gamma, \lock \qyields b : \Sharptype{B}}{\Gamma, \lock \qyields \SharpE{b} : B}
\end{mathpar}
