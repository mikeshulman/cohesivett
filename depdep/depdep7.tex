...
\inferrule*{\Gamma, \lock \qyields a : \Sharptype{A}}{\Gamma, \lock \qyields \SharpE{a} : A}
\end{mathpar}
%\begin{align}
%\id_\Gamma , \lock &\equiv \id_{\Gamma, \lock} \\
%(\Theta, \lock) ; (\kappa, \lock) &\equiv (\Theta ; \kappa), \lock \\
...
{\Gamma, \lock, \Ltype{A} \qyields \LE{c}[\comult{\Gamma} \uparrow \Ltype{A}] : C[\counit{\Gamma, \lock} \uparrow \Ltype{A}[\comult{\Gamma}]][\comult{\Gamma} \uparrow \Ltype{A}]}
\end{mathpar}
...
\item[\textsc{$\sharp$-elim}] For this we use the comultiplication:
\begin{mathpar}
\inferrule*[Left=cut]{
\inferrule*[Left=R-elim]
{\Gamma, \lock \qyields b : \Sharptype{B}}
{\Gamma, \lock, \lock \qyields \SharpE{b} : B}}
{\Gamma, \lock \qyields \SharpE{b}[\comult{\Gamma}] : B[\comult{\Gamma}]}
\end{mathpar}
\mvrnote{$B$ is not the same as $B[\comult{\Gamma}]$, we need to use the counit to put a $\lock$ on in formation/introduction}
\end{enumerate}
