...
\begin{align*}
\mathtt{whatever} : \sum_{\alpha : p} \El{p}{\alpha} \tcell \sum_{\alpha : p} \sum_{x : \El{p}{\alpha}} \El{p}{\alpha.x}
\end{align*}
with: 
\begin{align*}
...
\begin{align*}
\pinv{\alpha,x} : \TrPlus{\ApEl{p}{\pi^\alpha_x}}{\One_\alpha} \tcell_{\El{p}{\alpha.x}} \One_{\alpha.x} 
\end{align*}
such that
\begin{align*}
...
\fone{\alpha} : f_1(\One_\alpha) \tcell_{\El{q}{\TrPlus{f}{\alpha}}} \One_{\TrPlus{f}{\alpha}} \\
\foneinv{\alpha} : \One_{\TrPlus{f}{\alpha}} \tcell_{\El{q}{\TrPlus{f}{\alpha}}} f_1(\One_\alpha)
\end{align*}
such that
\begin{align}
...
%\mvrnote{Another option is a mode type morphism $q \tcell p$ rather than a term $p \yields f : q$, and then using the $s$-type. (The direction switch is a little confusing). In real cohesive HoTT, however, $\flat$ has only propositional eta, suggesting that using a term is the right thing.}
