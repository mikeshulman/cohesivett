...
\end{align*}
such that $\otimes_\alpha$ is symmetric and $1_\alpha$ is a unit for $\otimes_\alpha$. \mvrnote{(strictness?)}
\end{definition}

\begin{definition}
A comprehension object $p$ \emph{supports $\Sigma!$-types} if there is
\begin{align*}
\alpha : p, x : \El{p}{\alpha}, y : \El{p}{\alpha.x} \yields \Sigma!(x,y) : \El{p}{\alpha}
\end{align*}
and a mode term morphism
\begin{align*}
\alpha : p, x : \El{p}{\alpha}, y : \El{p}{\alpha.x} \yields y \tcell_{\El{p}{\alpha.x}} \TrPlus{\ApEl{p}{\pi^\alpha_x}}{\Sigma!(x,y)}
\end{align*}
\end{definition}
...
\mvrnote{Again, we could fix intro of ordinary $\Sigma$...}
\end{enumerate}
