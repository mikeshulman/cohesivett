...
\alpha$ and $\gamma \yields \nu : \beta$):
  \begin{itemize}
  \item A terminal object $\emptyset : p$ is given by $\emptyset :\equiv \TrPlus{\psi}{()}$
  \item Comprehension is given by $\alpha : p, x : \El{p}{\alpha} \yields \alpha.x :\equiv \TrPlus{\chi}{(\alpha, x)} : p$
  \item Mode term morphisms corresponding to projection and the variable rule are given via the counit of the adjunction $\chi^\circ \dashv \chi^+$.
\begin{align*}
