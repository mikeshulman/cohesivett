...
\end{align*}
such that $\otimes_\alpha$ is symmetric and $1_\alpha$ is a unit for $\otimes_\alpha$ \mvrnote{(strictness?)}. \mvrnote{Pending: What exactly do we need?}
\end{definition}
...
And the equations: \mvrnote{TODO}
%
%\begin{enumerate}[style = multiline, labelwidth = 80pt]
...
\mvrnote{DILTT} maintains the invariant that all types in the linear zone are well-formed in the intuitionistic context. A DILTT context $\Gamma \mid A, B, \dots$ is translated to a framework context $\upstairs{\Gamma}, a : \upstairs{A}, b : \upstairs{B}$ with $\Gamma$ translated as for MLTT, and with each of $\upstairs{A}$, $\upstairs{B}$ translated to types over $\El{p}{\modeof{\Gamma}}$.

As in ordinary MLTT, the translation of an object-level context $\Gamma \mid \Xi$ is equipped with a mode term morphism $\tshape{\Gamma} : p \tcell \ctxtuple{\downstairs{\Gamma}}$. We also require a term $\tfibshape{\Xi} : \El{p}{\modeof{\Gamma}}$ that describes the shape of the linear zone.
...
\inferrule*[left=term-lin-sub]{\Gamma \mid \Xi, B, \Xi'' \qyields a : A  \and \Gamma \mid \Xi' \qyields b : B}{\Gamma \mid \Xi, \Xi', \Xi'' \qyields a[b/B] : A} \\
\inferrule*[left=var]{~}{\Gamma, A \mid \cdot \qyields \qvar{A} : A[\proj{\Gamma, A}]} \and
