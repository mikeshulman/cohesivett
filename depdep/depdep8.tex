...
In previous work~\citep*{ls15adjoint,lsr17multi,lsr17multi-extended}, the
mode theory was a 2-category, or cartesian 2-multicategory.
...
work~\citep*{ls15adjoint,lsr17multi,lsr17multi-extended}, which were 2
...
In general, we think of the base type theory entities subscripting the
turnstile as ``permissions'' to use variables in certain ways, with the
mode theory dictating the ``policies'' of a particular type theory.  
...
In terms of permissions, weakening-over-weakening says that it is always
...
``permissions'' $\mu$, which is a separate step that must be given in
the mode theory.  Substitution-over-substitution says that, in the mode
...
In \citep{lsr17multi-extended}, we used an $n$-ary $\mathsf{F}$ type in
...
x:A.B$, where $\mu$ describes the permissions under which the variable
...
you how to combine $\nu_1$ (the permissions for the function) and
$\nu_2$ (the permissions for the argument) when you apply a function.
For example, in the simply typed case~\citep*{lsr17multi-extended}, taking
$\mu(c,x)$ to be $c \times x$ gives an $\to$ type right adjoint to a
product $\times$.    

We treat these as a negative type, with judgemental $\beta$ and $\eta$
equality rules.
...
\item A context $\qyields \Gamma \CTX$ is represented by a framework context $\upstairs{\Gamma}$ over mode context $\downstairs{\Gamma}$, together with a mode type morphism $\cdot \yields \tshape{\Gamma} : p \tcell \ctxtuple{\downstairs{\Gamma}}$ where $\ctxtuple{\downstairs{\Gamma}}$ is the iterated telescope type of Lemma~\ref{lem:ctxtuple}. This mode type morphism describes the dependency structure of the context.
  \msnote{Wouldn't it be more accurate to say that $\downstairs{\Gamma}$ describes the \emph{internal} dependency structure of the context (i.e.\ how types in $\Gamma$ depend on variables elsewhere in $\Gamma$) and $\tshape{\Gamma}$ describes the \emph{external} dependency structure (i.e.\ how some other judgment made \emph{in} context $\Gamma$ will depend on the variables in $\Gamma$)?  Or am I misunderstanding?}
