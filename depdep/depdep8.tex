...
2-category or 2-multicategory is a directed \emph{simple} type theory,
here we use a directed \emph{dependent} type theory as the
mode theory.
...
The mode theory can be thought of as a ``2-category with dependent
types,'' generalizing the base type theories of our previous
work~\cite{lsr17fibrational,ls15adjoint}, which were 2 (multi-)
categories.
...
In examples, we will often treat mode terms with a distinguished
parameter as though they were functions, because $f(\mu)$ is shorter
than $f[\mu/x]$.  This makes sense when $\gamma, x : p \yields f : q$
and the fact that it should be a function of $x$ is clear from context.

\subsection{Adjoint Mode Terms}

The base type theory is like a 2-category, and one can define an
adjunctions internally to any 2-category, so there is a definition
corresponding to an internal adjunction in the base type theory.
Adjunctions in the mode theory played a role in our previous
work~\cite{lsr17fibrational,ls15adjoint} because an adjunction in the
base generates to a triple adjunction in the top---this is because a
single mode term in the base generates an adjunction in the top, and an
adjoint pair of adjunctions is a triple adjunction.  Additional examples
of such triple adjunctions will come up here---for example as in the
Lawvere definition of quantifiers $\exists_x \dashv \pi_x \dashv
\forall_x$ in first-order logic.
...
contravariantly in the mode theory and $\TypeTwo{\gamma}{s}{p}{q}$ and
$\TermTwoT{\gamma}{s}{\mu}{\nu}{p}$ act \emph{contravariantly} on the
subscripts of upstairs terms.
