...
\newcommand{\pbaround}[3]{\mathrm{Pb}_{#1}(#2,#3)}
...
\inferrule*
    {\gamma \yields \mu : p}
    {\gamma \yields \TrPlus{\id_p}{\mu} \equiv \mu : p}

\inferrule*
    {\gamma \yields \mu : r \\
     \TypeTwo{\gamma}{s}{p}{q} \\
     \TypeTwo{\gamma}{s'}{q}{r} }
    {\gamma \yields \TrPlus{s}{\TrPlus{s'}{\mu}} \equiv
      \TrPlus{(s;s')}{\mu} : p}
\end{mathpar}
...
\\
\inferrule*{\TypeTwo{\gamma}{s}{p}{q}}{\TypeTwo{\gamma}{\id_p;s \equiv s \equiv s;\id_q}{p}{q}} \and

\inferrule*{\TypeTwo{\gamma}{s_1}{p_1}{p_2} \\\\
                \TypeTwo{\gamma}{s_2}{p_2}{p_3} \\\\
                \TypeTwo{\gamma}{s_3}{p_3}{p_4}
                }{\TypeTwo{\gamma}{(s_1;s_2);s_3 \equiv s_1;(s_2;s_3)}{p_1}{p_4}}
\\

\inferrule*{{\gamma,x:p} \vdash {q} \type}{\TypeTwo{\gamma}{\ap q {\id_{\mu}/x} \equiv \id_{q[\mu/x]}}{q[\mu/x]}{q[\mu/x]}} \and

\inferrule*{{\gamma,x:p} \vdash {q} \type \\ \TermTwoT{\gamma}{t}{\mu}{\mu'}{p} \\ \TermTwoT{\gamma}{t'}{\mu'}{\mu''}{p}}{\TypeTwo{\gamma}{\ap q {(t;t')/x} \equiv \ap q {t/x}; \ap q {t'/x}}{q[\mu/x]}{q[\mu''/x]} } \and
\inferrule*{\gamma \yields q \type \\ \TermTwoT{\gamma}{t}{\mu}{\mu'}{p}}{\TypeTwo{\gamma}{\ap q {t/\_} \equiv \id_q }{q}{q}} \\
\inferrule*{\gamma,y:p' \vdash \mu : p \and \gamma,x:p \vdash q \type \and \TermTwoT{\gamma}{t}{\nu}{\nu'}{p'}}{\TypeTwo{\gamma}{\ap {(q[\mu/x])} {t/y} \equiv \ap q {\ap \mu {t/y}/x}}{q[\mu/x][\nu/y]}{q[\mu/x][\nu'/y]}} \and
\inferrule*{\TypeTwo{\gamma,x:p}{t}{q}{q'} \and \TermTwoT{\gamma}{s}{\nu}{\nu'}{p}}{\TypeTwo{\gamma}{t[\nu/x];\ap{q'}{s/x} \equiv \ap{q}{s/x};t[\nu'/x]}{q[\nu/x]}{q'[\nu'/x]}}
%% subst: \id_\mu[\nu/x] = \id_{\mu[\nu/x]}
%% subst: s[x/x] = s
...
\\
\inferrule*{\TermTwoT{\gamma}{s}{\mu}{\mu'}{p} \and \gamma, x : p \yields q \type}{\TypeTwo{\gamma}{\ap{\id_q}{s/x} \equiv \ap{q}{s/x}}{q[\mu/x]}{q[\mu'/x]}} \and 
\inferrule*{\gamma, x : p \yields q \type \and \gamma, x : p \yields q' \type \and \TypeTwo{\gamma, x : p}{t}{q}{q'}}{\TypeTwo{\gamma}{\ap{t}{\id_{\mu}/x} \equiv t[\mu/x]}{q[\mu/x]}{q'[\mu/x]}}
\end{mathpar}
...
\\
\inferrule*{\TermTwoT{\gamma}{s}{\mu}{\mu'}{p}}{\TermTwoT{\gamma}{\id_\mu;s \equiv s \equiv s;\id_{\mu'}}{\mu}{\mu'}{p}} \and
\inferrule*{\TermTwoT{\gamma}{s_1}{\mu_1}{\mu_2}{p} \\\\
                \TermTwoT{\gamma}{s_2}{\mu_2}{\mu_3}{p} \\\\
                \TermTwoT{\gamma}{s_3}{\mu_3}{\mu_4}{p}
                }{\TermTwoT{\gamma}{(s_1;s_2);s_3 \equiv s_1;(s_2;s_3)}{\mu_1}{\mu_4}{p}} \\
\inferrule*{{\gamma,x:p} \yields {\nu} : {q} \and}{\TermTwoT{\gamma}{\ap \nu {\id_{\mu}/x} \equiv \id_{\nu[\mu/x]}}{\nu[\mu/x]}{\nu[\mu/x]}{q[\mu/x]}} \and
\inferrule*{\gamma, x : p \yields \nu : q \and \TermTwoT{\gamma}{s}{\mu}{\mu'}{p} \and \TermTwoT{\gamma}{t}{\mu'}{\mu''}{p}}{\TermTwoT{\gamma}{\ap \nu {(s;t)/x} \equiv \ap \nu {s/x} ; (\ap {(\TrPlus{\ap{q}{s/x}}{y})} {\ap \nu {t/x}/y})}{\nu[\mu/x]}{\TrPlus{\ap{q}{(s;t)/x}}{\nu[\mu''/x]}}{q[\mu/x]}} \\
\inferrule*{\TermTwoT{\gamma}{s}{\mu}{\mu'}{p}}{\TermTwoT{\gamma}{\ap x {s/x} \equiv s}{\mu}{\mu'}{p}} \\
\inferrule*{\gamma,x:p \vdash \nu : q \and \gamma,y:p' \vdash \mu : p \and \TermTwoT{\gamma}{s}{\xi}{\xi'}{p'}}{\TermTwoT{\gamma}{\ap {(\nu[\mu/x])} {s/y} \equiv \ap \nu {\ap \mu {s/y}/x}}{\nu[\mu/x][\xi/y]}{\TrPlus{\ap{q[\mu/x]}{s/y}}{\nu[\mu/x][\xi'/y]}}{q[\mu/x][\xi/y]}} \\
\inferrule*{\gamma \yields \nu : q \and \TermTwoT{\gamma}{s}{\mu}{\mu'}{p}}{\TermTwoT{\gamma}{\ap \nu {s/\_} \equiv \id_\nu}{\nu}{\nu}{q}} \\
\inferrule*{\TermTwoT{\gamma,x:p}{t}{\nu}{\nu'}{q} \\ \TermTwoT{\gamma}{s}{\mu}{\mu'}{p}}{\TermTwoT{\gamma}{t[\mu/x];\ap{\nu'}{s/x} \equiv \ap{\nu}{s/x};\ap{(\TrPlus{\ap{q}{s/x}}{y})}{t[\mu'/x]/y}}{\nu[\mu/x]}{\TrPlus{\ap{q}{s/x}}{\nu'[\mu'/x]}}{q[\mu/x]}} \\
\inferrule*{\TypeTwo{\gamma,x:p}{s}{q}{q'} \and
\TermTwoT{\gamma}{t}{\nu}{\nu'}{p} \and \gamma,x:p \vdash \mu : q'}{\TermTwoT{\gamma}{\ap{(\TrPlus{s}{\mu})}{t/x} \equiv \ApPlus{(s[\nu/x])}{\ap{\mu}{t/x}}}{\TrPlus{s}{\mu}[\nu/x]}{\TrPlus{\ap{q}{t/x}}{\TrPlus{s}{\mu}[\nu'/x]}}{q[\nu/x]}}
\end{mathpar}
...
\begin{mathpar}
  ...
  \inferrule*{ \gamma \yields p \type \\
               \gamma,x:p \yields q \type }{\TypeTwo{\gamma}{\sigmacl{x'}{\id_p}{\id_q} \equiv \id_{\sigmacl{x'}{p}{q}}}{\sigmacl{x}{p}{q}}{\sigmacl{x}{p}{q}}} \and
\inferrule*{\TypeTwo{\gamma}{s}{p}{p'} \\
    \TypeTwo{\gamma,x':p'}{t}{q[\TrPlus{s}{x'}/x]}{q'} \\\\ \TypeTwo{\gamma}{s'}{p'}{p''} \\
    \TypeTwo{\gamma,x'':p''}{t'}{q'[\TrPlus{s'}{x''}/x']}{q''}}{\TypeTwo{\gamma}{  \sigmacl{x'}{s}{t};\sigmacl{x''}{s'}{t'} \equiv \sigmacl{x''}{(s;s')}{(t[\TrPlus{s'}{x''}/x'];t')}}{\sigmacl{x}{p}{q}}{\sigmacl{x''}{p''}{q''}}}

\inferrule*{\gamma, y:r, \yields p \type \\
               \gamma, y:r, x:p \yields q \type \\
               \TermTwoT{\gamma}{s}{\mu}{\mu'}{r}
               }{\TypeTwo{\gamma}
               {\ap{\sigmacl{x'}{p}{q}}{s/y} \equiv
  \sigmacl{x'}{\ap{p}{s/y}}{\ap{({q[\fst z/x,\snd z/y]})}{\extend{s}{x'}/z}}}{\sigmacl{x'}{p}{q}[\mu/y]}{\sigmacl{x'}{p}{q}[\mu'/y]}}
  \\
\inferrule*{\TypeTwo{\gamma}{s}{p}{p'} \\
    \TypeTwo{\gamma,x':p'}{t}{q[\TrPlus{s}{x'}/x]}{q'} \and \gamma \yields \mu : \sigmacl{x'}{p'}{q'}}{\gamma \yields \TrPlus{\sigmacl{x}{s}{t}}{\mu} \equiv (\TrPlus{s}{\fst \mu},\TrPlus{(t[\fst \mu/x])}{\snd \mu}) : \sigmacl{x}{p}{q}}
\end{mathpar}
...
\inferrule*{\TermTwoT{\gamma}{s}{\mu}{\mu'}{\sigmacl{x}{p}{q}}}{\TermTwoT{\gamma}{s \equiv (\ap{\fst}{s}, \ap{\snd}{s})}{\mu}{\mu'}{\sigmacl{x}{p}{q}}}
\\
{\extend{\id_\mu}{\nu'}} \equiv \id_{(\mu,\nu')} \and
{\extend{(s;s')}{\nu''}} \equiv  \extend{s}{\TrPlus{\ap{q}{s'/x}}{\nu''}};\extend{s'}{\nu''}   \\
\inferrule*{\TermTwoT{\gamma}{s}{\mu}{\mu'}{p} \and \TermTwoT{\gamma}{t}{\nu'}{\nu''}{q[\mu'/x]}}{\TermTwoT{\gamma}{\extend{s}{\nu'} ; (\ap{(\mu',y)}{t/y}) \equiv
...
There are two derivable \mvrnote{Are they?} structural rules:
\begin{mathpar}
  \inferrule*[Left = var]{
 ...
 \inferrule*[Left = rewrite]{
  ...
  \inferrule*{\Gamma, x : B \yields_{\mu'} M : A
   \and \TermTwoT{\gamma}{s}{\mu}{\mu'}{p} \\\\
   \Gamma \yields_{\nu'} N : B
   \and \TermTwoT{\gamma}{t}{\nu}{\nu'}{q}}{\Gamma \yields_{\mu[\nu/x]} \rewrite{s}{M}[\rewrite{t}{N}/x] \equiv \rewrite{\ap{s}{t/x}}{\StI{\ap{q}{t/x}}{M[N/x]}} : A[\rewrite{t}{N}/x]}
\end{mathpar}
...
\begin{mathpar}
  ...
  \inferrule*[Left = F-beta]{
    \Gamma, y : \F{x.\mu}{A} \yields_{r} C \TYPE \and \Gamma, y : \F{x.\mu}{A} \yields_{r} C \ISFIB \\\\
    \Gamma \yields_{\nu} M : A \\\\
    \Gamma, x:A \yields_{\nu' [\mu / y]} N : C [\FI{x}/y]}
  {\Gamma \yields_{\nu'[\mu[\nu/x]/y]} \FE{\FI{M}}{x}{N} \equiv N[M/x] : C[\FI{M}/y]} \\  
...
%  \FE{M}{x}{N[\FI{x}/z]} \equiv N[M/z]
  ...
  \inferrule*[Left = F-Beta]{
    ...
    \Gamma \yields_{\nu} M : A \\\\
    \Gamma, x:A \yields_{r [\mu / y]} C \TYPE}
  {\Gamma \yields_{r[\mu[\nu/x]/y]} \FE{\FI{M}}{x}{C} \equiv C[M/x] \TYPE} \\  
...
%  \FE{M}{x}{C[\FI{x}/z]} \equiv C[M/z]
  ...
  \inferrule*{
    ...
    \Gamma \yields_{\nu} M : \F{x.\mu}{A} \\\\
    \Gamma, x:A \yields_{\nu''[\mu / y]} N : C [\FI{x}/y] \\\\
    \TermTwoT{\gamma, y : q}{t}{\nu'}{\nu''}{r}}
  {\Gamma \yields_{\nu'[\nu/y]}  (\FE{M}{x}{\rewrite{t[\mu/y]}{N}}) \equiv \rewrite{t[\nu/y]}{\FE{M}{x}{N}} : C[M/y]}
\end{mathpar}
...
substitution admissible.  The elimination rule pattern-matches on a term
...
\begin{mathpar}
  ...
  \inferrule*[Left = U-beta]{\Gamma,x:A \yields_{\mu[\nu_1/c]} M : B \\\\
                  \Gamma \yields_{\nu_2} N : A
  }{\Gamma \yields_{mu[\nu_2/x,\nu_1/c]} \UE{(\UI{x}{M})}{N} \equiv M[N/x] : B[N/x]}

  \inferrule*[Left = U-eta]{\Gamma \yields_\nu N : \U{c.\mu}{x:A}{B}}{\Gamma \yields_\nu \UI{x}{\UE{N}{x}} \equiv N : \U{c.\mu}{x:A}{B}}
...
\mvrnote{Text needs updating to refer to general E types}
...
\begin{mathpar}
  ...
  \inferrule*[Left = E-beta]{
    \Gamma, y : \E{x.\mu}{A} \yields_{r} C \TYPE \\\\
    \Gamma \yields_{\nu} M : A \\\\
    \Gamma, x:A \yields_{\nu' [\mu / y]} N : C [\EI{x}/y]}
  {\Gamma \yields_{\nu'[\mu[\nu/x]/y]} \EE{\EI{M}}{x}{N} \equiv N[M/x] : C[\EI{M}/y]} \\ 
  \inferrule*[Left = E-eta]{
  \Gamma, y : \E{x.\mu}{A} \yields_{r} C \TYPE \\\\
  \Gamma \yields_{\nu} M : \E{x.\mu}{A} \\\\ \Gamma, y : \E{x.\mu}{A} \yields_{\nu'} N : C}{\Gamma \yields_{\nu'[\nu/y]} (\EEs{\mu}{M}{x}{N[\EI{x}/y]}) \equiv N[M/y] : C[M/z]} \\
...
\inferrule*[Left = E-Elim]{
  ...
  {\Gamma \yields_{r[\nu/y]} \EEs{\mu}{M}{x}{C} \TYPE \quad (\text{over }  \gamma \yields {r[\nu/y]} \type)} \\
    ...
    \Gamma \yields_{\nu} M : A \\\\
    \Gamma, x:A \yields_{r [\mu / y]} C \TYPE}
  {\Gamma \yields_{r[\mu[\nu/x]/y]} \EE{\EI{M}}{x}{C} \equiv C[M/x] \TYPE} \\
  \inferrule*[Left = E-Eta]{
  \gamma,y:q \yields r \type \\\\
  \Gamma, y : \E{x.\mu}{A} \yields_{r} C \TYPE \\\\
  \Gamma \yields_{\nu} M : \E{x.\mu}{A}}{\Gamma \yields_{r[\nu/y]} (\EEs{\mu}{M}{x}{C[\EI{x}/y]}) \equiv C[M/y] \TYPE}
...
\mvrnote{We can now return to the teleport equation, need to label eq:stype-subst to reference }.
\begin{mathpar} 
  \inferrule*{\Gamma, x : B \yields_p A \TYPE \\\\
   \Gamma \yields_{\nu'} N : B
   \and \TermTwoT{\gamma}{t}{\nu}{\nu'}{q}}{\Gamma \yields_{p[\nu/x]} A[\rewrite{t}{N}/x] \equiv \St{\ap{q}{t/x}}{A[N/x]} \TYPE} \and 
  \inferrule*{\Gamma, x : B \yields_{\mu'} M : A
   \and \TermTwoT{\gamma}{s}{\mu}{\mu'}{p} \\\\
   \Gamma \yields_{\nu'} N : B
   \and \TermTwoT{\gamma}{t}{\nu}{\nu'}{q}}{\Gamma \yields_{\mu[\nu/x]} \rewrite{s}{M}[\rewrite{t}{N}/x] \equiv \rewrite{\ap{s}{t/x}}{\StI{\ap{q}{t/x}}{M[N/x]}} : A[\rewrite{t}{N}/x]}
...
\end{mathpar}
\mvrnote{This should be for all E-types?}
...
\msnote{I want to change the notation in the next theorem to match the syntax, writing $(\gamma,\Gamma)$ instead of $(A,a)$ and so on, but haven't done it yet.}
...
\begin{definition}
  ...
  \item \msnote{Something with $\Sigma$-types.}
...
\msnote{I don't know whether this morphism is automatically opcartesian, and can't even decide whether I think it probably is or isn't.
  But we almost certainly need to ask explicitly that it belong to the splitting.}
...
The relevant notion is a sort of mix of opcartesianness with a lifting property, which to our knowledge first appears explicitly in~\cite{CagneMellies2019} (\msnote{That paper isn't public yet, but there'll be a talk about it at HoTT 2019}), although it appears implicitly in the notion of ``fibration of tribes'' in~\cite{KapulkinSzumilo} (called a ``meta-fibration'' by Joyal).
...
\msnote{$\Fsym$-types don't have to be split functorial, but I presume there should still be some ``strict stability'' condition corresponding to preservation under syntactic substitution.}


\subsection{$\Usym$-types: cartesian arrows}
\label{sec:cartesian-arrows}

For $\Usym$-types, we need to similarly combine local cartesian closure with cartesian arrows.
To our knowledge the relevant definition does not appear in the literature, but it is fairly straightforward.

\begin{definition}[{\cite{Weber15}}]
  Let $f:A\to B$ and $g:Z\to A$ be morphisms in a category $\C$.
  A \textbf{pullback around $(f,g)$} is a commutative diagram
  \[
    \begin{tikzcd}
      X \ar[r,"p"] \ar[d,"q"'] & Z \ar[r,"g"] & A \ar[d,"f"]\\
      Y \ar[rr,"r"'] && B
    \end{tikzcd}
  \]
  such that the outer rectangle is a pullback.
  A morphism of pullbacks around $(f,g)$ is a pair of morphisms $s:X\to X'$ and $t:Y\to Y'$ making the obvious diagrams commute (i.e.\ $p' s = p$, $q s = t q'$, and $r = r' s$).
  This defines the category $\pbaround \C f g$.

  A \textbf{distributivity pullback} around $(f,g)$ is a terminal object of $\pbaround \C f g$.
\end{definition}

It is straightforward to verify that to give such a distributivity pullback around $(f,g)$ is equivalent to giving the morphism $r$ the structure of the image of $g$ under a right adjoint $\Pi_f$ to the pullback functor $f^*$, i.e.\ a dependent product of $g$ along $f$.

Now, given a functor $\vp:\C\to\M$, we want a notion of ``distributivity pullback relative to $\vp$''.
Of course if $\vp$ preserves pullbacks of $f$, it induces a functor $\pbaround\C f g \to \pbaround\M{\vp(f)}{\vp(g)}$.
Thus all we need is a notion of ``relative terminal object''.

\begin{proposition}\label{defn:rel-term}
  For a functor $\vp:\C\to\M$ and an object $x\in \C$ with $\vp(x)=v$, the following are equivalent.
  \begin{enumerate}
  \item The identity morphism $\vp(x)\to v$ is a universal arrow from the functor $\vp$.
  \item The identity morphism $\vp(x)\to v$ is a terminal object of the comma category $(\vp \downarrow v)$.
  \item For any $y\in \C$, the function $\C(y,x) \to \M(\vp(y),\vp(x))$ is a bijection.
  \item $x$ is a $\vp$-initial lift for the empty family of morphisms with domains $v$.
  \item (If $\vp$ is a fibration) $x$ is a terminal object of the fiber $\vp^{-1}(v)$ that is preserved by all the reindexing functors $f^*$ for morphisms $u\to v$ in $\M$.
  \end{enumerate}
\end{proposition}

When the conditions of Proposition \ref{defn:rel-term} are satisfied, we call $x$ a \textbf{codiscrete object relative to $\vp$}.
It is evident that being codiscrete relative to the functor $\C\to 1$ is the same as being terminal in $\C$.

\begin{definition}
  For a functor $\vp:\C\to\M$ and morphisms $f:A\to B$ and $g:Z\to A$ in $\C$, a \textbf{distributivity pullback around $(f,g)$ relative to $\vp$} is a pullback around $(f,g)$ that is codiscrete relative to the induced functor $\pbaround\C f g \to \pbaround\M{\vp(f)}{\vp(g)}$.
\end{definition}

\begin{definition}
  Let $\vp:\C\to\M$ be a local discrete fibration of 2-categories with families.
  Suppose given $\gamma\in \M$, $\Gamma\in \C$ with $\vp(\Gamma)=\gamma$, $p\in \Mty(\gamma)$, $A\in\Cty(\Gamma)$ with $\vp(A)=p$, $q\in \Mty(\gamma\ce p)$, $B\in \Cty(\Gamma\ce A)$ with $\vp(B)=q$, $r\in\Mty(\gamma)$, and a morphism $\mu : \gamma\ce p\ce r \to \gamma\ce p\ce q$ over $\gamma\ce p$.
  A \textbf{$\Usym$-type} for these data is a type $\U{\mu}{A}{B}\in \Cty(\gamma)$ with $\vp(\U\mu A B) = r$, together with a distributivity pullback around $(f,g)$ relative to (the underlying 1-functor of) $\vp$, where $f:\Gamma\ce A\to \Gamma$ and $g:\Gamma\ce A\ce B \to \Gamma\ce A$ are the projections:
  \[
    \begin{tikzcd}
      \Gamma \ce A\ce \U\mu A B \ar[r] \ar[d] & \Gamma\ce A \ce B \ar[r,"g"] & \Gamma\ce A \ar[d,"f"]\\
      \Gamma\ce \U\mu A B \ar[rr] && \Gamma
    \end{tikzcd}
    \hspace{1cm}\overset{\vp}{\mapsto}\hspace{1cm}
    \begin{tikzcd}
      \gamma\ce p \ce r \ar[r,"\mu"] \ar[d] & \gamma\ce p \ce q \ar[r] & \gamma\ce p \ar[d]\\
      \gamma\ce r \ar[rr] && \gamma
    \end{tikzcd}
  \]
  In the presence of a fibrancy structure, we require that $\U \mu A B$ is fibrant if both $A$ and $B$ are.
\end{definition}

Note that these outer rectangles are in fact pullbacks, arising from the comprehension of the reindexing of types in $\C$ and $\M$.
The morphism $\Gamma \ce A\ce \U\mu A B \to \Gamma\ce A \ce B$ over $\Gamma\ce A$ whose $\vp$-image is $\mu$ is the semantic version of the $\Usym$-elimination rule (``application'').
The $\Usym$-introduction rule, together with its beta and eta rules, are expressed by the universal property of the distributivity pullback relative to $\vp$.

From a categorical perspective, we would expect a 2-categorical aspect to the universal property as well.
But as in the case of $\Esym$-types, since $\vp$ is a local discrete fibration this comes for free.
\msnote{Should explain that some more.}

\msnote{Something about substitution stability too, I suppose.}
