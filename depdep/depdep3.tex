...
\item The functor $1$ at an object $\alpha$ is the term $[\alpha, 1_\alpha]$
\item The functor $\{-\}$ is given at $[\alpha, \mu]$ by $\alpha.\mu$.
\end{itemize}

We use $\pi$ and $\mathsf{var}_\mu$ to enforce the adjointness of $1$ and $\{-\}$. One way to express this adjoint is via a bijection 
\begin{mathpar}
\mprset{fraction={===}}
...
In what follows the first two equations are the $\beta$ principles for $\pi$ and $\mathsf{var}_\mu$ that guarantee the triangle commutes. The third is the $\eta$ principle that makes the factorisation unique.
\begin{align*}
...
\end{align*}
The following two equations are stricter than absolutely necessary, but seem convenient. The first is used to show that $1$ is functorial (only one direction is actually needed), and the second asserts that the unit of the adjunction is the identity.
\begin{align*}
s^+(1_\beta) &\equiv 1_\alpha \\
\alpha.1_\alpha &\equiv \alpha
\end{align*} 

If we add in $s^\circ$, the following equations seem reasonable:
\begin{align*}
...
\end{align*}
respectively. Cool! The long equation above, $\El{p}{s}^+(\El{p}{\pi}^\circ(\mu)) \equiv \El{p}{\pi}^\circ(\El{p}{(s, \mathsf{ap}(\El{p}{s}^+)(\id_x))}^+(\mu))$, is stating that $\Sigma$ as defined above is a morphism of fibrations.
