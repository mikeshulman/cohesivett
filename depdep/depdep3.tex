...
%% I don't really see how to do this in named form: 
...
\item The mode \emph{types} are
\begin{mathpar}
  \inferrule{ }
            {\gamma \yields p \type}
  \and
  \inferrule{\gamma \yields \mu : p}
            {\gamma \yields \El{p}{\mu} \type}
\end{mathpar}

\item The things that were mode types above are now mode terms of mode
  $p$:
  \begin{mathpar}
  \inferrule*[]{~}
               {\gamma \yields \emptyset : p} \and 

   \inferrule*[]{ \gamma \yields \mu : p \and
                  \gamma \yields \nu : \El{p}{\mu}}
               {  \gamma \yields (\mu.\nu) : p } 
  \end{mathpar}

\item The things that were mode terms of type $\alpha$ above are now
  mode terms of type $\El{p}{\mu}$ where $\mu : p$:
  \begin{mathpar}
  \inferrule*{ }
             {\gamma \yields 1 : \alpha}
             \qquad
...
\item To define $\Sigma,1,=$, add equations between mode terms of type
 $p$ (representing contexts) like
\[
\begin{array}{c}
  \mu.(\Sigma_\nu(\nu')) \equiv \mu.\nu.\nu' \\
  \mu.\nu.\TrPlus{w}{\nu'} \equiv \mu.\nu'.\TrPlus{w}{\nu} \\
...
\end{array}
\]
(for $=$ present unification via rewrite rules; not sure what that looks
like in de Bruijn form)

As well as equations between mode terms of type $\El{p}{\mu}$ pushing
substitution inside, and maybe
\[
\inferrule{\mu.\nu \equiv \mu.\nu' : p}
          {\nu \equiv \nu' : \El{p}{\mu}}
\]
...
\begin{mathpar}
  \inferrule*[]{  }
             {\TermTwoT{\gamma}{()}{\mu}{\emptyset}{p}}
             \and
  \inferrule*[]{ \TermTwoT{\gamma}{s}{\mu}{\mu'}{p} \and
                 \TermTwoT{\gamma}{t}{1}{\TrPlus{(\El{p}{s})}{\nu}}{\El{p}{\mu}}}
             {\TermTwoT{\gamma}{(s,t)}{\mu}{\mu'.\nu}{p}}
\end{mathpar}
...
\item The mode terms of type $p$ correspond to contexts.    
  \begin{mathpar}
  \inferrule*[]{~}
               {\gamma \yields \emptyset : p} \and 
  ...
  The mode 2-cells of type $p$ \TermTwoT{\gamma}{s}{\mu}{\mu'}{p}
  correspond to substitutions (maps in the context category of $p$),
  including projection and pairing:
  \begin{mathpar}
      \inferrule*[]{ \gamma \yields \mu : p \and
                  \gamma \yields \nu : \El{p}{\mu}
                }
                 {  \TermTwoT{\gamma}{\wk}{\mu.\nu}{\mu}{p} }
      \quad
      \inferrule*[]{ }
                 {\TermTwoT{\gamma}{\cdot}{\mu}{\emptyset}{p}}
      \quad
      \inferrule*[]{
        ...
        \TermTwoT{\gamma}{t}{1}{\TrPlus{\El{p}{s}}{\nu'}}{\El{p}{\mu}}
                 ...
                 {\TermTwoT{\gamma}{s,t}{\mu}{\mu'.\nu'}{p}}
  \end{mathpar}
  Here we write $1 : \El{p}{\mu}$ for $(\mu \mid 1 : \mu \vDash_p \mu)$
  using the constructor for dependent types defined next.

  \drlnote{TODO: equations}

  For $\TrPlus{s}{-}$, we also need the internal comprehension object's contexts to have
    pullbacks of arbitrary maps (where map means 2-cell in $p$):
  \begin{mathpar}
  \inferrule*
      {\gamma \yields \mu_1 : p \and 
        \TermTwoT{\gamma}{s_1}{\mu_1}{\mu}{p} \and
        \TermTwoT{\gamma}{s_2}{\mu_2}{\mu}{p}
      }
    {\gamma \yields \mu_1 \times^{s_1,s_2}_\mu \mu_2 : p}
  \end{mathpar}
  ...
  \inferrule*
    ...
    {\TermTwoT{\gamma}{\fst}{\mu_1 \times^{s_1,s_2}_\mu \mu_2}{\mu_1}{p}}
    \qquad
  \inferrule*
      {\gamma \yields \mu_1 : p \and 
        \TermTwoT{\gamma}{s_1}{\mu_1}{\mu}{p} \and
    ...
    {\TermTwoT{\gamma}{\snd}{\mu_1 \times^{s_1,s_2}_\mu \mu_2}{\mu_2}{p}}
  \end{mathpar}

  \drlnote{TODO: unviersal property}

  \drlnote{TODO: add equations reducing the pushout}
  
\item The mode terms of type $\El{p}{\mu}$ where $\mu : p$ correspond to
  dependent types in context $\mu$, and are the things that upstairs
  \emph{term} judgements will be indexed by.

  The desired action $\TrCirc{\El{p}{s : \mu \vDash_p \mu'}} {\nu :
    \El{p}{\mu}} : \El{p}{\mu'}$ together with having a unit type $1 :
  \El{p}{\mu}$ means that \emph{every} term 2-cell $s : \mu \vDash_p
  \mu'$ induces a dependent type over $\mu'$.  So it seems simplest to
  literally represent the dependent types by another mode term of type
  $p$ (context/upstairs type dependency) paired with a 2-cell:
  \begin{mathpar}
    \inferrule*{ \gamma \yields \mu' : p \and
                 \TermTwoT{\Gamma}{s}{\mu'}{\mu}{p} \and
                }
               {\gamma \yields (\mu' \mid s) : \El{p}{\mu}}
  \end{mathpar}
  \drlnote{Unsure about 2-cell in $p$ vs type 2-cell here: premise could
  also be $\TypeTwo{\Gamma}{s}{\El{p}{\mu'}}{\El{p}{\mu}}$.}

  Any dependent type can be $\eta$-expanded into its projection, and
  comprehending with an explicit map $\beta$-reduces:
  \begin{mathpar}
   \inferrule*[]{ \gamma \yields \mu : p \and
                  \gamma \yields \nu : \El{p}{\mu}
                }
   ...
   \quad
    \inferrule*{ \gamma \yields \mu' : p \and
               ...
               {\gamma \yields \mu.(\mu' \mid s) \equiv \mu' : p}
   \quad
    \inferrule*{ \gamma \yields \mu' : p \and
                 \TermTwoT{\Gamma}{s}{\mu'}{\mu}{p} \and
                }
               {\TermTwoT{\gamma}{\wk_{\mu,(\mu' \mid s)} \equiv s}{\mu.(\mu' \mid s)}{\mu}{p}}
  \end{mathpar}

  Recall that all mode theories have terms for variables,
  $\TrPlus{s}{-}$ and $\TrCirc{s}{-}$.  For variables we don't add any
  special equations, but the above gives
  \begin{mathpar}
  \inferrule*{x : \El{p}{\mu} \in \gamma}
             {\gamma \yields x \equiv (\mu.x \mid \wk_{\mu,x}) : \El{p}{\mu}}
  \end{mathpar}

  For $\TrCirc{s}{-}$ we have that it's implemented by composition:
  \begin{mathpar}
  \inferrule*
    {\gamma \yields (\mu'' \mid s'') : \El{p}{\mu} \\
     \TermTwoT{\gamma}{s'}{\mu}{\mu'}{p}
    }
    {\gamma \yields \TrCirc{\El{p}{s'}}{\mu'' \mid s''} \equiv (\mu'' \mid s'';s') : \El{p}{\mu'}}
  \end{mathpar}
  \drlnote{If the only type 2-cells
  \TypeTwo{\gamma}{s}{\El{p}{\mu}}{\alpha}
  are $\El{p}{s'}$ for $s'$ as above, then this is a complete
  ...
  \begin{mathpar}
    ...
    {\gamma \yields \TrPlus{s'}{\mu'' \mid s''} := (\mu'' \times_\mu \mu' \mid \mathsf{snd}) : \El{p}{\mu'}}
  \and
  \end{mathpar}

  FIXME: what does closure under substitution mean?

\item We might have some freedom to pick fewer of these, but it seems
  like it works to take term 2-cells in $\El{p}{\mu}$ to be term 2-cells
  in $p$ that commute with projections:
  \begin{mathpar}
    \inferrule*{
                 \TermTwoT{\gamma}{s}{\mu.\nu}{\mu.\nu'}{p} \and
                 s;\wk_{\mu,\nu'} \equiv \wk_{\mu,\nu'} 
...
%% \item The mode terms of type $p$ correspond to contexts.  Contexts are
%%   restricted to normal form---you can't add a $\Sigma$.  
%%   \begin{mathpar}
%%   \inferrule*[]{~}
%%                {\gamma \yields \emptyset : p} \and 

%%    \inferrule*[]{ \gamma \yields \mu : p \and
%%                    x : \alpha \in \gamma \and
%%                    \TypeTwo{\gamma}{s}{\El{p}{\mu}}{\alpha}
%%                 }
%%                {  \gamma \yields (\mu, i : \TrPlus{s}{x}) : p } 
%%   \end{mathpar}

%%   The mode 2-cells of type $p$ \TermTwoT{\gamma}{s}{\mu}{\mu'}{p}
%%   correspond to substitutions (maps in the context category of $p$).  
%%   Some of those are ``display maps''.  We need a pullback of a display
%%   map and an arbitrary map 
%%   \begin{mathpar}
%%     \inferrule*
%%     { \TermTwoT{\gamma}{s'}{\mu'}{\mu}{p} \and
%%       \TermTwoT{\gamma}{s''}{\mu''}{\mu}{p} \and
%%       \mathsf{display}(s'')
%%     }
%%     {\gamma \yields \mu'' \times_\mu \mu' : p}
%%   \end{mathpar}
%%   If all contexts are comprehensions as above, this is definable, but
%%   I'm not sure what happens with modalities.
  
%% \item The mode terms of type $\El{p}{\mu}$ where $\mu : p$ correspond to
%%   dependent types in context $\mu$.  We represent them as a chosen
%%   class of ``display'' substitutions/context morphisms.  
%%   \begin{mathpar}
%%     \inferrule*{ \gamma \yields \mu' : p \and
%%                  \TermTwoT{\Gamma}{s}{\mu'}{\mu}{p} \and
%%                  \mathsf{display}(s)
...
%%   We need at least the following display maps: projections from a
%%   context extension, pullback of a display map along an arbitrary map,
%%   composition of display maps:
%%   \begin{mathpar}
%%   \inferrule*{x : \El{p}{\mu} \in \gamma}
%%              {\gamma \yields x := (\mu, i : \TrPlus{1}{x} \mid \mu/\mu) : \El{p}{\mu}}
%%              \and
%%   \inferrule*
%%     {\gamma \yields (\mu'' \mid s'') : \El{p}{\mu} \\
%%      \TermTwoT{\gamma}{s'}{\mu'}{\mu}{p}
...
%%      \TermTwoT{\gamma}{s'}{\mu}{\mu'}{p}
%%     }
%%     {\gamma \yields \TrCirc{s'}{(\mu'' \mid s'')} := (\mu'' \mid s'';s') : \El{p}{\mu'}}
%%   \end{mathpar}

%%   Note: assuming \TypeTwo{\gamma}{s}{\El{p}{\mu}}{\El{p}{\mu'}}
%%   iff \TermTwoT{\gamma}{s}{\mu}{\mu'}{p}; otherwise need to use the
%%   former.  

%%   FIXME: closure under substitution
...
%% \subsection{Axiomatic Strict Internal-Comprehension-Category Mode Theory
%% (Universey, de Bruijn, Strict Contraction)}
...
%%   We leave off $=$ because $\TermTwoT{\gamma}{-}{1}{\nu}{\El{p}{\mu}}$
%%   will be a mere proposition (set containment).
  
%% \item To define $\Sigma,1$ give equations between mode terms of type $p$
...
%% As well as equations between mode terms of type $\El{p}{\mu}$ pushing
%% substitution inside, and 
%% \[
%% \inferrule{\mu.\nu \equiv \mu.\nu' : p}
%%           {\nu \equiv \nu' : \El{p}{\mu}}
%% \]
%% to inherit associativity, etc.  

%% For $\TermTwoT{\gamma}{s}{\mu}{\mu'}{p}$, first rewrite both contexts
%% into normal form according to the above, so that both sides are of the
%% form $x_1,\TrPlus{w_2}{x_2},\ldots,\TrPlus{w_n}{x_n}$, where $w_i$ shows
%% that all dependencies of $x_i$ are in the prefix, and then check that
%% each $x_i'$ in $\mu'$ is also in $\mu$.  

%% For $\TermTwoT{\gamma}{s}{\nu}{\nu'}{\El{p}{\mu}}$, just check
%% $\TermTwoT{\gamma}{s}{\mu.\nu}{\mu.\nu'}{p}$, because if it's
%% propositional, then it will commute with the projections to $\mu$
%% automatically.

%% It should be possible to phrase this as a quotienting of the syntax.
%% \end{enumerate}
...
\\
  ...
  \inferrule{\gamma \yields \alpha : p \and \gamma \yields \mu : \El{p}{\alpha}}
            {\TermTwoT{\gamma}{\pi}{\alpha.\mu}{\alpha}{p}}
\\
...
\end{itemize}
In our notation, the type family $\El{p}{-}$ is the fibration $p$, the term $1_\alpha$ is the functor $1$ at $\alpha$, and $\{\mu\}$ is given by $\alpha.\mu$. We will use $\pi$ and $\mathsf{var}_\mu$ to enforce the adjointness of $1$ and $\{-\}$. 

One way to express this adjoint is via a bijection 
\begin{mathpar}
\mprset{fraction={===}}
...
\end{mathpar}
The two premises correspond to a single morphism $(\beta, 1_\beta) \to (\alpha, \mu)$ in $\mathcal{E}$. Rephrasing this using the universal morphism perspective of adjunctions, the claim is that any morphism $(s, t) : (\beta, 1_\beta) \to (\alpha, \mu)$ in $\mathcal{E}$ factors as:
\[\begin{tikzcd}
(\beta, 1_\beta) \arrow[dr, "{(s, t)}" swap] \arrow[rr, dashed, "{((s,t), \id_{1_\beta})}"] && (\alpha.\mu, 1_{\alpha.\mu}) \arrow[dl, "{(\pi, \mathsf{var}_\mu)}"] \\
& (\alpha, \mu)
\end{tikzcd}\]

The first two equations are the $\beta$ principles for $\pi$ and $\mathsf{var}_\mu$ that guarantee the triangle commutes. The third is the $\eta$ principle that makes the factorisation unique.
\begin{align*}
...
\end{align*}
The following two equations are stricter than absolutely necessary, but seem convenient. The first is used to show that $1$ is functorial (only one direction is actually needed), and the second asserts that the unit of the adjunction $1 \dashv \{-\}$ is the identity.
\begin{align*}
s^+(1_\beta) &\equiv 1_\alpha \\
\alpha.1_\alpha &\equiv \alpha
\end{align*} 

If we add in $s^\circ$, the following guesses seem reasonable:
\begin{align*}
...
\end{align*}
The first two are justified by thinking about the profunctor version, and the third one is expressing that $\pi$ is a morphism of fibrations.
