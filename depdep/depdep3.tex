...
Just like any mode theory, we still have the following (a reminder):
\begin{mathpar}
  \inferrule{\TermTwoT{\gamma}{s}{\alpha}{\beta}{p} \and \gamma \yields \mu : \alpha \and \gamma \yields \nu : \beta \and \TermTwoT{\gamma}{m}{\mu}{\El{p}{s}^+(\nu)}{\El{p}{\alpha}}}
            ...
            \\
\inferrule*
    {\gamma \yields \alpha : q \\
     \TypeTwo{\gamma}{s}{p}{q}
    }
    {\gamma \yields \TrPlus{s}{\alpha} : p}
    
\inferrule*
    {\gamma \yields \alpha : p \\
     \TypeTwo{\gamma}{s}{p}{q}
    }
    {\gamma \yields \TrCirc{s}{\alpha} : q} \\

\inferrule*
    {\TypeTwo{\gamma}{s}{\alpha}{\beta} \and (x : \beta \in \gamma)
    }
    {\TermTwoT{\gamma}{\varepsilon_{s,x}}{\TrCirc{s}{\TrPlus{s}{x}}}{x}{\beta}}

\inferrule*
    {\TypeTwo{\gamma}{s}{\alpha}{\beta} \and (y : \alpha \in \gamma)
    }
    {\TermTwoT{\gamma}{\eta_{s,y}}{y}{\TrPlus{s}{\TrCirc{s}{y}}}{\alpha}}
\end{mathpar}

Now we get the term for $\Sigma$ types:
\begin{align*}
...
\end{align*}
If we apply the equations below, these terms are:
\begin{align*}
...
What equations do we need? Some of the following? Here I am writing the composition of 2-cells as $s;t$, instead of using substitution notation. 

And I am writing $\mathsf{ap}$ of a function on a 2-cell as follows: if $\TermTwoT{\gamma}{s}{\beta}{\alpha}{p}$ and $\TermTwoT{\gamma}{m}{\mu}{\nu}{\El{p}{\alpha}}$ then $\mathsf{ap}(\El{p}{s}^+)(m)$  is a term 2-cell $\TermTwoT{\gamma}{\mathsf{ap}(\El{p}{s}^+)(m)}{\El{p}{s}^+(\mu)}{\El{p}{s}^+(\nu)}{\El{p}{\beta}}$.
