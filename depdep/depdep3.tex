...
\begin{mathpar}
\inferrule*[]{(i : x[\sigma]) \in \alpha \and \gamma \mid \beta \yields \tau : \alpha}{(i : x[\sigma[\tau]]) \in \beta}
\end{mathpar}
\end{lemma}
\mvrnote{This is already sloppy, I suppose we need to properly define $(i : x[\sigma]) \in \alpha$, and deal with prefixes $\alpha' \leq \alpha$ of types...}

\begin{lemma}
Composition for renamings is admissible:
\begin{mathpar}
\inferrule*[]{\gamma \mid \beta \yields \sigma : \alpha \and \gamma \mid \delta \yields \tau : \beta}{\gamma \mid \delta \yields \sigma[\tau] : \alpha}
\end{mathpar}
\end{lemma}
...
Now inductively we have $\gamma \mid \delta \yields \sigma'[\tau] : \alpha'$, and we reapply the renaming-extension rule with $(j : x[\theta [\sigma'][\tau]]) \in \delta$. \mvrnote{Here we have to use associativity of reaming composition}
\end{proof}
...
\begin{lemma}
Given $(i : x[\tau]) \in \alpha$ and $(i' : x[\tau]) \in \alpha$, there is a type $\alpha_{i=i'}$ where the two variables have been identified.  \mvrnote{Is there any difference in what we did compared to just using a contraction renaming?}
\end{lemma}
...
\begin{lemma}
Universal property for pushout.
\end{lemma}
\begin{proof}
\mvrnote{todo}
\end{proof}

The following all simultaneously. In what follows, if $\gamma \yields \mu : \alpha$ is a term, let $\gamma \yields \bar \mu \yields \mu : \alpha$ be its underlying renaming.

\begin{lemma}\label{inl-sub}
$\inl$ for substitution:
\begin{mathpar}
\inferrule*[]{\gamma, x_0 : \alpha_0, \gamma' \mid \alpha \yields \tau : \alpha_0 \and (i : x_0[\tau]) \in \alpha \and \gamma \yields \mu_0 : \alpha_0}{\gamma, \gamma'[\mu_0 / x_0] \mid \alpha[\mu_0 / x_0] \yields \sigma' : \bar \mu_0}
\end{mathpar}
such that $\mu_0[\sigma'] = \tau[\mu_0/x_0]$
\end{lemma}
\begin{proof}
%Induction on $\bar \mu_0$. If $\bar \mu_0 = \emptyset$, then also $\alpha_0 = \emptyset$, and the equation holds trivially.

Induction on $\alpha_0$. If $\alpha_0 = \emptyset$, then $\tau$ is the empty renaming, and the equation holds trivially as renamings into $\emptyset$.

In the inductive case we have
\begin{align*}
\alpha_0 &= (\alpha_0, i : x[\theta]) && \text{where } (x : \beta) \in \gamma \text{ and } \gamma \mid \alpha_0 \yields \theta : \beta \\
\mu_0 &= (\mu_0, j/i) && \text{where } (j : x[\theta[\mu_0]]) \in \bar \mu_0
\end{align*}
(Note that we cannot have $x = x_0$ as $\alpha_0$ is only a type in context $\gamma$.)

Inductively we have $\gamma, \gamma'[\mu_0 / x_0] \mid \alpha[\mu_0 / x_0] \yields \sigma' : \bar \mu_0$.
...
\bar \mu_0 \ar[r] \ar[drr, bend right, "\text{Lemma \ref{inl-sub}}" swap]& \Pushout{\alpha[\mu_0/x_0]}{\alpha_0}{\bar \mu_0}{\tau[\mu_0/x_0]}{\mu_0} \ar[dr, dashed] & \\
& & \beta[\mu_0/x_0]
\end{tikzcd}
\]
Where Lemma \ref{inl-sub} is applied to $(j : x_0[\tau[\sigma]]) \in \beta$.
\end{proof}
