...
mode, but only notation for declaring a function symbol in the signature
(it cannot occur on the right-hand side of a typing judgement).  For
example, the term constructors for a monoid $(\odot,1)$ are represented
by a signature
\[
\dsd{p} \, \dsd{mode},
...
equational axioms will be used to encode reversible object logic
...
transformations will be used to represent object logic structural
...
the term formers) closed under axioms the specified in the signature
$\Sigma$.  For example, to say that the above monoid $(\odot,1)$ is
affine, we add in $\Sigma$ a transformation axiom $(x \spr 1 : (x:p) \to
{p})$.
...
the signature $\Sigma$ in the judgement forms.
