...
\framebox{Context descriptors/mode morphisms \oftps{\psi}{\Sigma}{\alpha}{p},
...
\framebox{Structural transformations/mode 2-morphisms
  \wfsps{\psi}{\Sigma}{\alpha}{\alpha'}{p},
same presuppositions as \deq
}
...
new mode declaration, or a signature extended with a typed constant, all
of whose modes are declared previously in the signature.  The notation
$p_1,\ldots,p_n \to q$ is not itself a mode, but only notation for
declaring a function symbol in the signature.  For example, the term
constructors for a monoid $(\odot,1)$ are represented by a signature
\[
\dsd{p} \, \dsd{mode},
\dsd{\odot} : (\dsd{p},\dsd{p} \to \dsd{p}),
1 : (\to \dsd{o})
\]

The judgement $\psi \, \dsd{ctx}_\Sigma$ defines well-formedness of a
context of variable delcarations relative to a signature $\Sigma$: each
mode in the context must be declared in the signature.

The judgement $\oftps{\psi}{\Sigma}{\alpha}{p}$ defines well-typedness
of context descriptor terms, which are either a variable declared in the
context or a constant declared in the signature applied to arguments of
the correct types.  The judgement $\oftps{\psi}{\Sigma}{\gamma}{\psi'}$
...
The final two rules of the judgement $\wfsig{\Sigma}$ permit two
additional forms of signature declaration.  The first extends a
signature with an equational axiom between two terms that have the same
mode $p$, in the same context $\psi$, relative to the prior signature
$\Sigma$.  These equational axioms will be used to encode object logic
structural properties, such as associativity, commutativity, and unit
laws.  For example, to specify the left unit law for the above monoid
$(\odot,1)$, we would add an axiom $x \odot 1 \deq x : (x : \dsd{p}) \to
\dsd{p}$ to the signature.  The judgement
\deqtms{\psi}{\Sigma}{\alpha}{\alpha'}{p} is the least congruence closed
under these axioms.

The second extends a signature with a directed structural transformation
axiom between two terms that have the same mode $p$, in the same context
$\psi$, relative to the prior signature $\Sigma$.  As discussed above,
these structural transformations will be used to represent object logic
structural properties such as weakening and contraction that are not
invertible.  The judgement \wfsps{\psi}{\Sigma}{\alpha}{\alpha'}{p}
defines these transformations: it is the least precongruence (preorder
compatible with the term formers) closed under axioms the specified in
the signature $\Sigma$.  For example, to say that the above monoid
$(\odot,1)$ is affine, we would specify in $\Sigma$ a transformation
axiom $(x \spr 1 : (x:p) \to {p})$.
...
\spr \alpha$.  
%% (and, writing $s_1;s_2$ for derivations by the above
%% transitivity rule, equations $s;s' = 1_{\alpha}$ and $s';s =
%% 1_{\alpha'}$)
While this is conceptually and technically sufficient, we have found it
helpful in examples to use ``strict'' equality of context descriptors.
This simplifies the description of some situations, though the
difference is important mainly at the level of identity of derivations
rather than provability---for example, we can make a binary operation
$\odot$ into a strict monoid, rather than adding associator and unitor
isomorphisms.  Technically, because context descriptors $\alpha$ and
their equality $\alpha_1 \deq \alpha_2$ are defined prior to the
subsequent judgements of our logic, we suppress this equality by using
...
In examples, we will typically introduce a context descriptor constant
by writing it applied to variables, e.g. writing $x : \dsd{p}, y :
\dsd{p} \vdash x \odot y : \dsd{p}$ for $\odot : \dsd{p},\dsd{p} \to
\dsd{p}$.  We also suppress the signature $\Sigma$ in the judgement
forms, as it is fixed at the outside of all derivations.  
