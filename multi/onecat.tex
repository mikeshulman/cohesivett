...
Define the \emph{shape} of a derivation of \seq{\Gamma}{\alpha}{A} or
\seq{\Gamma}{\gamma}{\Delta} to be the corresponding term built from the
grammar
\[
s :: \dsd{var} \mid \F L(s) \mid \F R(s) \mid \U L(s) \mid \U R(s) \mid \cdot \mid (s_1,s_2)
\]
(i.e., write the proof term corresponding to the derivation, and then
discard the evidence that variables are in a context and the evidence
for equations).  Shapes are technically necessary for the cut proof,
where we sometimes weaken a derivation before applying the inductive
hypothesis.
...
derivation has the same shape as the given one.  
\end{theorem}
\begin{proof}
