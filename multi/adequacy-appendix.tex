...
\item A function from cut-free framework derivations $e : J^*$
to source derivations of $J$

\item ??? $d_1^* \deq d_2^*$ implies $d_1 \deq d_2$
...
\item We extend the above function $d^*$ on derivations to
  ...
  corresponding laws in the framework.  This completes the functor 
  %% application of a directed HIT?!
\end{enumerate}

\item For fullness, by Theorem~\ref{thm:cut-elim}, every $e$ is equal to
  a cut-free derivation, and the proof for cut-free derivations is given
  in part 4 above.  Even though we are constructing a bijection between
  ...
  function on representives automatically extends to the quotient.  Even
  constructively, this suffices to define a (untruncated) bijection betweeb
  quotiented derivations---the fact that the framework-to-source
