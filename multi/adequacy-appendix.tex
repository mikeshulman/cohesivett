...
\item For fullness, every $e$ is equal to a
  cut/identity/transformation-free derivation $\elim{e}$, and the proof
  for cut-free derivations is given by \backtrf{-}.  Even though we are
  constructing a bijection between derivations modulo \deq, we do not
  need to show that this function respects the quotient: because of the
  ``mere existence''/$-1$-truncation in the definition of surjective,
  the function on representives automatically extends to the quotient.
...
\item For faithfulness, we need to show that $d_1^* \deq d_2^*$ implies
  $d_1 \deq d_2$.  By part 7 above, it suffices to show 
  ...
  so part 8 above gives the result.
\end{enumerate}
\end{remark}
