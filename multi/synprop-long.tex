...
structural properties does not contribute to the size).  Sizes are
necessary for the cut proof, where we sometimes weaken or invert a
derivation before applying the inductive hypothesis.

\begin{lemma}[Respect for Transformations] ~ \label{lemma:respectspr}
\begin{enumerate}
...
structural properties are ignored by the size.  The cases for $\FL$ and
$\UR$ use the inductive hypothesis, along with congruence for structural
properties to show that $\subst{\beta}{\alpha}{x} \spr
\subst{\beta'}{\alpha}{x}$ or $\subst{\alpha}{\beta}{x} \spr
\subst{\alpha}{\beta'}{x}$.  The cases for substitutions rely on the
fact that no generating structural properties for mode substitutions are
...
structural properties, and the inductive hypotheses.  The third
...
\]
where the structural property is the composition of the
\subst{-}{\alpha_0}{x_0} substitution into the given structural
property, and rearranging the substitution (note that $x_0$ does not
occur in $\alpha$):
\[
\begin{array}{ll}
...
Lemma~\ref{lemma:respectspr} gives the result.  If $z$ is not $x_0$,
...
$x$ in scope.  Thus, we use Lemma~\ref{lemma:Finv} to pull the
...
\]
The structural property premise is
\[
...
\]
So applying Lemma~\ref{lemma:respectspr} gives 
...
{\subst{\subst{\beta'}{\alpha_0}{x_0}}{\tsubst{\alpha}{\subst{\gamma}{\alpha_0}{x_0}}}{z}}
\]
so Lemma~\ref{lemma:respectspr} gives the result.  
...
\subst{\subst{\beta'}{\tsubst{\alpha}{\gamma}}{z}}{\alpha_0}{x_0}
\]
by congruence on the assumed structural property, and then commute substitutions.  
...
The types respect the structural properties, covariantly for \Fsymb\/
and contravariantly for \Usymb\/.

\begin{lemma}[Types Respect Structural Properties] ~ \label{lem:}
\begin{enumerate}
\item 
