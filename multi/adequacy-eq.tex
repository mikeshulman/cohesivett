...
\begin{lemma}[Strengthening Fusion] \label{lem:0-use-strengthening-fusion}
...
%% But $\Linv{\D}{(y,z)}{x} = \Cuta{\D}{\FR{(y,z)}}{x}$, 
%% %% is left inverting literally the same as cutting?
%% %% if not prove that backtranslation takes it to cutting.  
%% and $\backtrf{\Cuta{\D}{\FR{(y,z)}}{x}} \deq 
%% \backtrf{\D}[\backtrf{\FR(y,z)}/x]$
